...
# Transmon qubit 中的噪声：从 $T_1$、$T_2$、$T_\phi$ 到实验室中的噪声源与测量

## 0. 基本对象：Transmon 与“噪声如何进入哈密顿量”
Transmon 的核心是约瑟夫森结与电容形成的弱非简谐振子。其本征频率（近似）为
$$
\omega_{01}\approx \frac{1}{\hbar}\left(\sqrt{8E_JE_C}-E_C\right).
$$

噪声通常通过某个外参量 $\lambda$ 的涨落进入系统：
- 电荷/电压噪声（影响 $n_g$ 或岛电势）
- 磁通噪声（影响 $E_J(\Phi)$）
- 临界电流噪声（影响 $E_J$）
- 与环境电磁场的耦合（通过电偶极/电流偶极耦合到外部阻抗、谐振子、传输线）

抽象地写，系统-环境耦合常可表示为
$$
\hat H = \hat H_q + \hat H_{\mathrm{env}} + \hat H_{\mathrm{int}},\qquad
\hat H_{\mathrm{int}}=\hat A\,\hat X_{\mathrm{env}},
$$
其中 $\hat A$ 是作用在比特上的算符（例如 $\hat n$、$\hat\varphi$、$\hat\sigma_x$、$\hat\sigma_z$ 相关），$\hat X_{\mathrm{env}}$ 是环境噪声自由度（电压、电流、磁通等）。

---

## 1. 三个关键时间：$T_1$、$T_2$、$T_\phi$ 的关系与含义

### 1.1 能量弛豫时间 $T_1$
$T_1$ 描述从 $|1\rangle\to|0\rangle$ 的能量损失，通常由**高频噪声**（在 $\omega_{01}$ 附近）驱动：
$$
T_1=\frac{1}{\Gamma_1},\quad \Gamma_1=\Gamma_\downarrow+\Gamma_\uparrow.
$$
极低温下常有 $\Gamma_\uparrow\approx 0$，因此 $T_1\simeq 1/\Gamma_\downarrow$。若环境存在热占据 $n_{\mathrm{th}}$，则会出现可观的上跃迁并带来残余激发态布居。

在“算符耦合 + 噪声谱密度”的形式下（费米黄金律）：
$$
\Gamma_\downarrow
=\frac{1}{\hbar^2}\left|\langle 0|\hat A|1\rangle\right|^2 S_X(+\omega_{01}),
$$
其中 $S_X(\omega)$ 是环境噪声的（单边/双边）谱密度，具体定义取决于约定，但物理点是：**$T_1$ 由噪声在 $\omega_{01}$ 附近的谱密度决定**。

### 1.2 相干时间 $T_2$ 与纯退相干 $T_\phi$
$T_2$ 描述相干项 $\rho_{01}$ 的衰减。对两能级马尔可夫模型，有经典关系：
$$
\frac{1}{T_2}=\Gamma_2=\frac{\Gamma_1}{2}+\Gamma_\phi
\quad\Longleftrightarrow\quad
\frac{1}{T_\phi}=\Gamma_\phi=\frac{1}{T_2}-\frac{1}{2T_1}.
$$

- $\Gamma_1/2$：来自能量弛豫的不可避免退相干（“放光就丢相位”）
- $\Gamma_\phi$：**纯退相干**，通常由**低频噪声**（近 DC 或 kHz–MHz）导致频率抖动：$\omega_{01}(t)=\omega_{01}+\delta\omega(t)$

### 1.3 实验上常见的 $T_2^\*$、Ramsey 与 Echo
实验上会区分：
- $T_2^\*$（Ramsey）包含慢漂移/准静态噪声导致的“非均匀展宽”
- $T_2^{\mathrm{echo}}$（spin echo）能滤除一部分低频噪声，更接近“真正的”退相干速率

因此常见现象是
$$
T_2^\* \le T_2^{\mathrm{echo}} \le 2T_1.
$$

---

## 2. Transmon 的主要噪声机制（按物理来源分类）

下面列出实验室里最常遇到、且最影响 $T_1/T_2$ 的噪声源。不同实验平台权重不同，但几乎都绕不开。

---

## 2.1 电磁环境耦合与辐射损耗（外部线缆/阻抗）
### (a) 通过控制线、读出谐振子泄露：阻抗型耗散
Transmon 通过电容（或电感）耦合到外部电路。外部电路在 $\omega_{01}$ 处的等效阻抗/导纳的实部提供耗散通道：
- **外部导纳 $\mathrm{Re}\,Y(\omega_{01})$ 越大，$T_1$ 越差**
- 这也是为什么要在低温端大量使用滤波、衰减、隔离、以及阻抗工程

### (b) Purcell 衰减（通过读出腔泄露）
当比特与读出谐振子色散耦合时，腔的线宽 $\kappa$ 会在比特频率处打开辐射通道，形成 Purcell 衰减。最常用近似：
$$
\Gamma_{\mathrm{Purcell}}\approx \left(\frac{g}{\Delta}\right)^2\kappa
\quad (\Delta=\omega_q-\omega_r).
$$
工程上常用 **Purcell filter** 通过让环境在 $\omega_q$ 处“看起来更反应性/更小实部导纳”来抑制该通道，同时保持读出带宽。

---

## 2.2 材料与界面损耗：介质损耗、表面参与比、TLS
这是当代 transmon 中非常常见的 $T_1$ 限制因素之一：
- 介质（基底、氧化层、空气-金属-介质界面）中的损耗正切（loss tangent）
- 两能级缺陷系统 TLS 与电场耦合，导致能量吸收与频率漂移
- 典型特征：$T_1$ 随频率变化起伏明显；对温度/功率/应力敏感；器件间差异大

实验上常通过：
- 扫频测 $T_1(\omega_q)$ 找到“损耗峰/避交叉”
- 改变几何（更大电容板、更低表面参与比）来改善

---

## 2.3 准粒子（Quasiparticles）
超导中非平衡准粒子可穿隧约瑟夫森结，导致：
- 能量弛豫（降低 $T_1$）
- 频率抖动/额外退相干
...
典型诊断：
- $T_1$ 的“突发性跳变”（burst），随后缓慢恢复
- 与温度、屏蔽、封装、辐射环境相关

---

## 2.4 磁通噪声（Flux noise）
对 SQUID transmon（可调频）尤其重要。磁通噪声常呈 $1/f$：
$$
S_\Phi(f)\approx \frac{A_\Phi^2}{f},
$$
它通过频率对磁通的敏感度耦合进来：
$$
...
关键实验事实：
- 在“甜点”（sweet spot）$\partial\omega_q/\partial\Phi\approx 0$ 处，低频磁通噪声导致的退相干显著减弱，$T_2^\*$ 会明显变长
- 远离甜点，$T_2^\*$ 常被磁通 $1/f$ 噪声限制，echo 能部分改善但不一定完全救回来（取决于噪声谱）

实验室来源包括：
- 电流源噪声/偏置线噪声（DAC、AWG、偏置源、接地环路）
- 磁屏蔽不足（外界磁场漂移）
- 封装/材料磁性杂质
...
## 2.5 电荷噪声与临界电流噪声
Transmon 设计的初衷之一就是降低对电荷噪声敏感度（大 $E_J/E_C$ 使电荷色散指数压低），因此在 0–1 跃迁上电荷噪声通常不是主导项，但在更高能级或特定器件仍可能显现。

临界电流噪声表现为 $E_J$ 的涨落：
$$
\delta\omega_q \approx \frac{\partial\omega_q}{\partial E_J}\,\delta E_J.
$$
它可能来自结的微观涨落、TLS 与结电导的耦合等，通常较难直接分离。

---

## 2.6 “光子噪声”与测量相关噪声（读出腔中的残余光子）
### (a) 测量诱导退相干（measurement-induced dephasing）
在色散读出中，比特态决定腔的响应（指针态不同），测量本质上把相位信息泄露到输出场，导致退相干。常用表达（形式之一）：
$$
\Gamma_\phi^{\mathrm{meas}}=\frac{\kappa}{2}\,|\alpha_e-\alpha_g|^2,
$$
其中 $\alpha_{g/e}$ 是比特处于 $|g\rangle/|e\rangle$ 时腔内稳态相干幅度。

因此：
- 增大读出功率（腔内光子数 $\bar n$）通常会增大 $\Gamma_\phi^{\mathrm{meas}}$
- 读出要在“足够快/足够强”与“不过度破坏相干”之间权衡（尽管 QND 读出可在一定程度上兼顾）

### (b) 残余热光子（residual thermal photons）导致的“光子散粒噪声退相干”
即使不测量，腔里若有残余 $\bar n_{\mathrm{th}}$，其数目涨落会通过色散位移 $\chi$ 引起比特频率抖动，从而退相干：
- 实验室里常见原因：输入线热化不充分、隔离不足导致 300K 或 4K 的噪声“漏到腔里”
- 典型表现：$T_2^{\mathrm{echo}}$ 也不理想，且随衰减器配置、隔离器方向、HEMT 反向噪声等显著变化

### (c) dressed dephasing / 测量引发的伪跃迁
在色散区，低频退相干噪声可在失谐频率 $\Delta$ 附近被“上/下变频”，导致测量期间比特发生非期望跃迁（常体现为读出中的 state transition、QND 性变差）。这在强读出或噪声环境较差时更明显。

---

## 3. “噪声与耦合”的统一视角：噪声谱与滤波
把“耦合强弱”与“噪声大小”统一起来的语言是：**环境噪声谱密度 $S(\omega)$ 与耦合算符矩阵元**。

- $T_1$ 关心 $S(\omega_{01})$（GHz 附近）
- $T_\phi$ 多由 $S(\omega\approx 0)$（近 DC 的 $1/f$）或 MHz 噪声决定
- 读出与驱动会改变系统对噪声的“滤波函数”（例如 spin echo、CPMG、spin-locking 可把低频噪声映射到特定频段以做噪声谱学）

工程手段实质上是在做：
1) 减小耦合（降低矩阵元或降低 $\mathrm{Re}\,Y$）  
2) 降低噪声源谱密度（降温、衰减、滤波、隔离）  
3) 重新分配噪声进入系统的通道（Purcell filter、模式工程、封装与接地）

---

## 4. 实验室中最常见的“噪声入口”（非常实用的清单）

### 4.1 输入/输出微波线（XY 线、读出线）
常见问题：
- 室温噪声沿线下传 → 腔/比特热激发、$T_2$ 变差
- HEMT 反向噪声回灌（若隔离不足）
- 连接器/开关/线缆驻波 → 频率依赖的 $T_1$ 起伏

常用对策（按从 300K 到 mK 的链路）：
- 分级衰减（例如 20 dB@4K + 20 dB@100 mK + 20 dB@mixing）
- 低通/带通/红外滤波（Eccosorb、Powder filter、RC/铜粉等）
- 隔离器/环行器（在输出链路上隔离 HEMT 噪声）
- 良好接地与屏蔽，避免地环路把低频噪声灌入

### 4.2 DC/Flux 偏置线
常见问题：
- 电流源噪声（尤其低频）→ Ramsey 快速退相干
- 地噪声、开关电源纹波、DAC 杂散
- EMI/50 Hz 及谐波
...
- mK 端 RC/铜粉滤波 + 低温电阻热化
- 仪器端电池供电或低噪声电源、隔离放大器
- 磁屏蔽（Cryoperm/铅屏蔽）与布线去环路

### 4.3 封装与辐射（红外、盒模、缝隙模）
常见问题：
- 红外泄露产生准粒子 → $T_1$ 降低且不稳定
- 封装腔体模式与缝隙模耦合 → 额外损耗通道与串扰
对策：
- 红外吸收/黑化处理、屏蔽罩
- 优化封装与接地、抑制缝隙模（airbridge/跨接/接地针）
- 盒模仿真与模式管理

---

## 5. 如何在实验上“测到这些噪声”：典型测量与诊断路径

### 5.1 测 $T_1$（弛豫）
流程：$\pi$ 脉冲激发到 $|1\rangle$，延迟 $\tau$ 后读出，拟合
$$
P_1(\tau)=P_1(0)\,e^{-\tau/T_1}+P_{1,\mathrm{eq}}.
$$
诊断思路：
- 扫 $\omega_q$（调频 transmon）测 $T_1(\omega_q)$：识别 Purcell 限制、TLS 峰、封装模式
- 改变读出腔耦合/滤波器：验证 Purcell 通道
- 改变温度：看是否受热激发、准粒子影响

### 5.2 测 $T_2^\*$（Ramsey）与 $T_2^{\mathrm{echo}}$
- Ramsey：$\pi/2$—等待—$\pi/2$，相位随 detuning 进动，包络给 $T_2^\*$
- Echo：$\pi/2$—$\tau/2$—$\pi$—$\tau/2$—$\pi/2$，包络给 $T_2^{\mathrm{echo}}$

经验判断：
- 若 $T_2^\*$ 很短但 echo 提升明显 → 低频（$1/f$）噪声主导（磁通/偏置噪声常见）
- 若 echo 也不高，且接近 $2T_1$ → 可能主要被 $T_1$ 限制
- 若 echo 仍远小于 $2T_1$ → 可能有中频噪声（kHz–MHz）、热光子、驱动泄露等

### 5.3 测 $T_\phi$（从 $T_1,T_2$ 推出）
测得 $T_1$ 与某个 $T_2$ 后用
$$
\Gamma_\phi=\frac{1}{T_\phi}=\frac{1}{T_2}-\frac{1}{2T_1}
$$
估算纯退相干强度，并通过改变工作点（sweet spot）、线缆滤波、读出功率来定位来源。

### 5.4 与测量相关的诊断
- 扫读出功率：若 $T_2$ 或测量期间跳变随 $\bar n$ 明显变差 → 测量诱导退相干/伪跃迁/热光子问题
- 检查腔的 ring-down：读出脉冲尾巴会在后续门操作期间造成额外退相干（需优化脉冲、加主动清腔、或提高 $\kappa$ 并配合滤波）

---

## 6. 总结：把 $T_1/T_2/T_\phi$ 与“噪声频段”对上号
- $T_1$：主要看 **GHz（$\omega_{01}$）** 的耗散通道  
  Purcell、外部线缆阻抗、介质/TLS、准粒子、辐射与封装模等
- $T_\phi$（以及 $T_2^\*$）：主要看 **低频到 MHz** 的频率噪声  
  磁通 $1/f$、偏置线噪声、热光子数涨落、驱动/读出泄露
- 测量系统本身既是“观察者”也是“噪声源”：读出强度、隔离与热化决定了测量相关退相干与残余激发

---

如果你能提供：你的 transmon 类型（固定频/可调）、典型参数（$\omega_q,\omega_r,g,\Delta,\kappa$）、线缆衰减与隔离配置、以及测得的 $T_1,T_2^\*,T_2^{\mathrm{echo}}$ 随工作点/读出功率的变化，我可以按“最可能的噪声主导项”给出更针对的定位步骤与改进清单。
