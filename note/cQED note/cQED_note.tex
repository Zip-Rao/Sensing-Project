...
\textbf{旋转波近似有效的条件（旋转参考系频率为$\omega_q$）}：\(\hbar\omega_q \gg E_C\)，这在 transmon
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\linewidth]{img/LC coupler.png}
  \caption{transmon 与 LC 谐振器的等效电路}
\end{figure}
...
g不为零时，qubit与光子相互作用开启，文中给出 dressed 态可由幺正变换从裸态获得： \[
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\linewidth]{img/JC spectrum.png}
  \caption{JC 模型能谱示意（文中 Fig.8）}
\end{figure}
...
% ======================================================================
\section{与外界耦合：环境的作用（Coupling to the outside world: the role of the environment）}
% ======================================================================

到目前为止讨论的是\textbf{孤立量子系统}。但对量子电路的完整描述必须包含系统如何与环境耦合（测量装置、控制线路等）。环境在量子技术中具有双重角色：
\begin{itemize}
  \item \textbf{不可避免性}：完全隔离不现实，因为总会存在非期望的环境自由度耦合。
  \item \textbf{必要性}：完全隔离也"没用"，因为我们将无法控制、驱动或测量系统。
\end{itemize}

因此本节考虑：量子系统与\textbf{外部传输线}耦合，并引入对电路QED读出至关重要的\textbf{输入--输出理论（input-output theory）}。

\bigskip\hrule\bigskip

% ======================================================================
\subsection{用传输线连接量子系统（Wiring up quantum systems with transmission lines）}
% ======================================================================

% --------------------------------------------------
\subsubsection{传输线作为环境：连续谱极限}
% --------------------------------------------------

考虑单个量子系统（示例：LC振子）通过电容耦合到一根\textbf{半无限}传输线（如共面波导，CPW）。
\begin{itemize}
  \item 传输线长度 $d\to \infty$ 时，模频率变得\textbf{极其稠密}，需视为\textbf{连续谱（continuum）}。
  \item 传输线哈密顿量写为连续模积分形式：
\end{itemize}
...
% --------------------------------------------------
\subsubsection{传输线的场变量（通量与电荷）的连续模展开}
% --------------------------------------------------
...
\begin{itemize}
  \item 这里 $v=1/\sqrt{lc}$ 为传输线相速度；$l,c$ 分别是单位长度电感、电容（注意与后文的耦合电容符号区分）。
  \item 在海森堡绘景下，$\hat\Phi_{\mathrm{tml}}(x,t)=e^{i\hat H_{\mathrm{tml}}t/\hbar}\hat\Phi_{\mathrm{tml}}(x)e^{-i\hat H_{\mathrm{tml}}t/\hbar}$（$\hat Q$ 同理）。
\end{itemize}

% --------------------------------------------------
\subsubsection{通过电容将传输线耦合到谐振子（LC）}
% --------------------------------------------------
...
\end{equation}
式中，\begin{itemize}
  \item $d\omega$的积分表示外部环境（传输线）的模连续遍历所有频率，
  \item $\lambda$为耦合强度分布，通常在系统频率$\omega_r$附近占主导。
  \item 最后一项：系统与传输线的耦合，$\lambda(\omega)$ 是频率依赖的耦合强度。
  \end{itemize}
耦合强度的频率依赖形式为
\begin{equation}
...
\begin{itemize}
  \item $C_r$ 为谐振子电容；
  \item $C_t$ 为传输线等效电容（文中以"归一化/有效电容"方式出现）；
  \item 这些表达式可视作因耦合导致的电容小重整化（附录A讨论）。
\end{itemize}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{img/LC coupled to TL.png}
\caption{LC 振子通过电容 $C_k$ 与半无限传输线耦合的示意图。}
\end{figure}
% --------------------------------------------------
\subsubsection{窄带/高Q与旋转波近似：得到标准系统--浴耦合形式}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{Born--Markov 近似：Lindblad 主方程（谐振子衰减与热激发）}
% --------------------------------------------------
...
\begin{itemize}
  \item $\kappa$：谐振子的能量衰减率（线宽）。文中给出
  \begin{equation}
  \kappa=2\pi\lambda(\omega_r)^2
  = Z_{\mathrm{tml}}\omega_r^2\frac{C_k^2}{C_r},
  \end{equation}
  其中 $Z_{\mathrm{tml}}$ 为传输线特性阻抗。
  \item $\bar n_\kappa=\bar n_\kappa(\omega_r)$：传输线在频率 $\omega_r$ 处的热光子数（Bose--Einstein 分布给出），并满足浴相关函数
  \begin{equation}
...
\begin{itemize}
  \item $\mathcal D[\hat a]$ 项：对应\textbf{光子损失}（衰减）。当作用在数态 $\lvert n\rangle$ 上，$\hat a\lvert n\rangle=\sqrt n\lvert n-1\rangle$，体现 $n\to n-1$ 的跃迁。
  \item $\mathcal D[\hat a^\dagger]$ 项：对应从热浴\textbf{吸收光子}导致的激发。
  \item $T\to 0$（稀释制冷极低温）常取 $\bar n_\kappa\to 0$，但实际可能因室温线缆热辐射等导致残余热占据，需要工程抑制（吸收器、滤波、隔离等）。
\end{itemize}

\bigskip\hrule\bigskip

% ======================================================================
\subsection{电网络中的输入--输出理论（Input-output theory in electrical networks）}
% ======================================================================

% --------------------------------------------------
\subsubsection{为什么需要输入--输出理论}
% --------------------------------------------------
...
本节介绍一种常用推导路线：将传输线模分解为\textbf{左行（输入）}与\textbf{右行（输出）}辐射场，并用 $x=0$ 处边界条件联系二者。

% --------------------------------------------------
\subsubsection{左/右行场与端口电压算符}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{边界条件（Kirchhoff 定律）与端口电流}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{标准输入--输出关系与输入/输出场定义}
% --------------------------------------------------

通过对相关算符做模展开并采用前述近似（系统仅响应 $\omega\approx\omega_r$，忽略快速旋转项，可将积分范围扩展到 $(-\infty,\infty)$），得到\textbf{标准输入--输出关系}：
...
\begin{quote}
\textbf{关键近似}：丢弃 $\pm\omega_r$ 的快速旋转项（RWA/窄带），并假设相关频率都靠近 $\omega_r$，从而得到 Markov 极限的 $\delta(t-t')$ 关联。
\end{quote}

% --------------------------------------------------
\subsubsection{海森堡--朗之万方程：谐振子由输入场驱动并以 $\kappa$ 衰减}
% --------------------------------------------------
...
含义：
\begin{itemize}
  \item $-\kappa\hat a/2$：辐射阻尼（能量泄露到线中）
  \item $\sqrt{\kappa}\hat b_{\mathrm{in}}$：输入场（噪声/外加驱动）对系统的驱动
  \item 结合(72)可由输入与系统动力学预测输出，从而与实验可测信号建立联系。
\end{itemize}

% --------------------------------------------------
\subsubsection{传输线中某位置的电压（可测信号表达式）}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{扩展性：从单端口到量子网络}
% --------------------------------------------------

本节主要讨论单个系统连接半无限传输线端点的简单情形。更一般地：
\begin{itemize}
  \item 多个系统可耦合到同一条线形成网络；
  \item 多条线可作为多个端口。
\end{itemize}
这些更复杂结构可用 \textbf{SLH 形式化}系统化处理（文中提及 Combes、Gough \& James 等）。

\bigskip\hrule\bigskip

% ======================================================================
\subsection{量子比特的弛豫与退相干（Qubit relaxation and dephasing）}
% ======================================================================

% --------------------------------------------------
\subsubsection{从谐振子到一般量子系统：主方程的普适性}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{Transmon 的能量弛豫（连接传输线导致的衰减）}
% --------------------------------------------------

对 transmon（图12）：
\begin{itemize}
  \item 将式(68)中 $\hat H_S$ 替换为 transmon 哈密顿量 $\hat H_q$（文中指向前面 Eq.(25)）。
  \item 并将耗散算符替换为 transmon 的跃迁算符：
  \begin{itemize}
    \item $\mathcal D[\hat a]\to \mathcal D[\hat b]$
    \item $\mathcal D[\hat a^\dagger]\to \mathcal D[\hat b^\dagger]$
  \end{itemize}
  \item 同时将 $\kappa\to\gamma$，其中
  \begin{equation}
...
其中：
\begin{itemize}
  \item $\rho$ 现在是 transmon 的密度矩阵；
  \item $\bar n_\gamma$ 是量子比特环境在 $\omega_q$ 处的热占据数。常取 $\bar n_\gamma\to 0$，但实验上也常观察到残余热激发。
\end{itemize}

% --------------------------------------------------
\subsubsection{纯退相干（pure dephasing）的表型模型}
% --------------------------------------------------
...
\begin{itemize}
  \item 对 0--1 跃迁的 transmon，因其对电荷噪声不敏感，$\gamma_\phi$ 往往较小；但高能级因电荷色散随能级数增长而变得更明显，可能出现更强退相干。
  \item 另一个退相干来源：与读出谐振子色散耦合时，谐振子中残余热光子数的涨落会引起量子比特频率抖动（"光子数涨落退相干"）。在这种情况下，也可在谐振子主方程中加入类似式(78)但以 $\hat a^\dagger \hat a$ 替代的项；通常该贡献较小而常被忽略。
  \item 还存在材料/界面双能级系统（TLS）、准粒子、红外与电离辐射等引起的弛豫与退相干来源。
\end{itemize}

% --------------------------------------------------
\subsubsection{合并后（取 $\bar n_\gamma\to 0$）的 transmon 主方程}
% --------------------------------------------------
...
% --------------------------------------------------
\subsubsection{$T_1$ 与 $T_2$：弛豫与相干时间}
% --------------------------------------------------
...
\begin{itemize}
  ...
  \tag{80a}
  \end{equation}
  $T_1$为qubit从激发态弛豫到基态的特征时间。其中在有限温度时 $\gamma_\downarrow=(\bar n_\gamma+1)\gamma$、$\gamma_\uparrow=\bar n_\gamma\gamma$；零温近似下 $T_1\simeq 1/\gamma$。
  \item 横向退相干时间（相干寿命）：
  \begin{equation}
...
\begin{itemize}
  \item \textbf{能量弛豫}过程往往可较好地用 Markov 主方程描述；
  \item \textbf{退相干}（尤其由低频噪声导致）常具有显著非Markov特征，Markov 处理对退相干只是某种"粗略近似"，但在预测稳态响应等场景仍常有效。
\end{itemize}


% ======================================================================
\subsection{色散区的耗散（Dissipation in the dispersive regime）}
% ======================================================================

本节讨论 \textbf{transmon--谐振子} 在色散耦合区（dispersive regime）下的耗散描述。关键点：在色散区里，系统的本征态是\textbf{修饰态（dressed states）}，比特与腔并非真正独立；因此即使耗散通道看似"分别作用在比特/腔"，修饰结构也会导致\textbf{交叉耗散效应}（例如 Purcell 衰减、dressed dephasing）。

% --------------------------------------------------
\subsubsection{"独立浴"近似下的复合系统主方程（bare basis 的直觉式）}
% --------------------------------------------------
...
\begin{itemize}
  \item $\rho$：\textbf{总系统}密度矩阵
  \item $\hat H$：耦合的 transmon--谐振子哈密顿量（文中指向前面 Eq.(32)）
  \item $\kappa$：腔衰减率（光子泄露）
  \item $\gamma$：比特能量弛豫率
  \item $\gamma_\phi$：比特纯退相干率
  \item $\hat a$：谐振子湮灭算符；$\hat b$：transmon 的跃迁/降低算符（在弱非简谐下近似类振子）
\end{itemize}

\textbf{重要限制/提醒}：式(81)只对 $\omega_q/\omega_r$ 的小值区间有效。更本质的原因是：真实能量衰减发生在\textbf{系统本征态之间}的跃迁；而式(81)的耗散子更像是在未耦合的"裸态"（bare states）之间写跃迁，因而不是在任意耦合强度/任意频率比下都可靠。（文中提到一般推导可见 Beaudoin et al.\ 2011）
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{img/dissipation in coupled system.png}
\caption{transmon 与谐振子耦合后与环境耦合示意图}
\end{figure}
\bigskip\hrule\bigskip

% --------------------------------------------------
\subsubsection{修饰态导致的交叉耗散：色散变换后的主方程}
% --------------------------------------------------

乍看式(81)似乎表示腔耗散与比特耗散彼此独立。但在色散区，由于修饰态将 $|e,0\rangle$ 与 $|g,1\rangle$ 混合（图13），腔的阻尼可能诱导比特跃迁，反之亦然。
...
&\quad +2\gamma_\phi \mathcal D[\hat b^\dagger\hat b]\rho_{\mathrm{disp}}
...
\paragraph{新出现的三类修正速率（Purcell/反向Purcell/测量诱导跃迁）}

式(82)中出现三种"交叉"速率（文中首先给出白噪声近似下的简单形式）：

\begin{equation}
\gamma_\kappa=\left(\frac{g}{\Delta}\right)^2\kappa,\qquad
\kappa_\gamma=\left(\frac{g}{\Delta}\right)^2\gamma,\qquad
\gamma_\Delta=2\left(\frac{g}{\Delta}\right)^2\gamma_\phi .
\tag{83}
\end{equation}

并且文中强调式(82)"有三个新速率"，其中第一个是 \textbf{Purcell 衰减率}：

\begin{itemize}
  \item \textbf{Purcell 衰减率 $\gamma_\kappa$}：腔的损耗 $\kappa$ 通过修饰态混合导致比特弛豫。直觉图像：修饰激发态近似为
  \begin{equation}
  ...
  \item \textbf{反向效应 $\kappa_\gamma$}：同理，比特的衰减通道也会导致腔的有效衰减修正（图13亦标注为 inverse Purcell）。
  \item \textbf{最后一行（含 $\gamma_\Delta$ 的两项）更微妙}：它们对应由比特退相干噪声在色散耦合下"上/下变频"，从而诱导比特在读出时发生非预期跃迁的过程（见下）。
\end{itemize}

\bigskip\hrule\bigskip

% --------------------------------------------------
\subsubsection{消去腔自由度得到"dressed dephasing"（读出引发的伪跃迁）}
% --------------------------------------------------
...
\begin{itemize}
  \item 弛豫/激发速率 $\sim \bar n\,\gamma_\Delta$
\end{itemize}

这在实验上常表现为量子比特测量期间的\textbf{非期望状态跃迁}，可解释为：退相干噪声在失谐频率 $\Delta$ 处被读出光子上/下变频，导致"dressed dephasing"并引发跃迁。

\bigskip\hrule\bigskip

% --------------------------------------------------
\subsubsection{更精细推导：耗散率的频率依赖与"噪声谱"视角}
% --------------------------------------------------

文中指出：上面对式(82)的"直接把色散变换作用到主方程"的捷径忽略了耗散率的频率依赖。更严格的做法是对\textbf{系统+浴的哈密顿量}先做色散变换，再推导主方程（Boissonneault et al.\ 2009）。这样得到的形式仍类似式(82)，但速率表达式会改变，并显示出"在不同频率探测环境"的物理：

\begin{itemize}
  \item 记 $\kappa=\kappa(\omega_r)$，$\gamma=\gamma(\omega_q)$，强调：
  \begin{itemize}
    \item 腔光子弛豫"探测"环境在 $\omega_r$ 处的噪声/阻抗
    \item 比特弛豫"探测"环境在 $\omega_q$ 处的噪声/阻抗
  \end{itemize}
\end{itemize}
...
\end{equation}
从而更清楚地看到：\textbf{Purcell 衰减是"在比特频率 $\omega_q$ 处发射一个光子"，而不是在腔频率 $\omega_r$ 处}（这也解释了白噪声近似可能造成的概念混淆）。
...
\begin{quote}
\textbf{dressed dephasing 实际上在探测 transmon--谐振子失谐频率 $\Delta$ 处的噪声。}
\end{quote}
...
% --------------------------------------------------
\subsubsection{与纵向耦合（longitudinal coupling）的对比}
% --------------------------------------------------

本节结论依赖于 Jaynes--Cummings 型电偶极耦合导致的修饰（dressing）。若将相互作用替换为纵向耦合（文中指向 Eq.(63) 形式），则不会产生同样的光--物质修饰，因此：

\begin{itemize}
  \item \textbf{无 Purcell 衰减}
  \item \textbf{无 dressed dephasing}
\end{itemize}
...
% ======================================================================
\subsection{多模 Purcell 效应与 Purcell 滤波器（Multi-mode Purcell effect and Purcell filters）}
% ======================================================================

% --------------------------------------------------
\subsubsection{从单模到多模：Purcell 率的求和与发散问题}
% --------------------------------------------------

前面讨论的是比特色散耦合到\textbf{单模}谐振子。若谐振子是多模结构（多模腔/传输线谐振器），则"对各模的 dressing"会带来 Purcell 率的多模累加。朴素类比会得到修正速率形如
\begin{equation}
\sum_m \left(\frac{g_m}{\Delta_m}\right)^2 \kappa_m,
\end{equation}
其中 $m$ 为模编号。

然而若进一步考虑 $f_m$、$g_m$、$\Delta_m$ 的频率依赖，该求和在某些模型下会出现\textbf{发散}（Houck et al.\ 2008）。因此需要更精细的模型（考虑 transmon 的有限尺寸、谐振子输入/输出电容的频率依赖阻抗等；Bourassa 2012; Malekakhlagh 2017）以得到物理上良好的结果。

\bigskip\hrule\bigskip

% --------------------------------------------------
\subsubsection{用"环境导纳"计算 Purcell：工程视角更直接}
% --------------------------------------------------
...
其中
\begin{itemize}
  \item $Y(\omega)=1/Z(\omega)$ 为 transmon "看到的"电磁环境的导纳
  \item $C_\Sigma$、$C_s$ 为与器件电容相关的参数（文中给出该形式以强调比例关系与"探测环境"的思想）
\end{itemize}
...
% --------------------------------------------------
\subsubsection{Purcell 滤波器（Purcell filter）的思想与实现}
% --------------------------------------------------

一种常见做法是在谐振子的输出端加入合适长度并在开路端终止的传输线支节（transmission-line stub），使得在比特频率处形成抑制辐射的响应。这种结构称为 \textbf{Purcell filter}（Reed et al.\ 2010b）。

其效果（与图13文字一致）：
\begin{itemize}
  \item 在不显著牺牲读出需要的腔耗散/带宽的情况下，
  \item \textbf{降低比特频率处的"腔修饰态密度"与环境耦合}，
  \item 从而抑制 Purcell 衰减。
\end{itemize}
...
% ======================================================================
\subsection{用微波驱动控制量子系统（Controlling quantum systems with microwave drives）}
% ======================================================================

连接到传输线会引入损耗，但对控制与测量是必要的。本节用输入--输出框架描述经典微波驱动。

% --------------------------------------------------
\subsubsection{连续波驱动的输入场建模：$b_{\mathrm{in}}(t)$ 加上经典部分}
% --------------------------------------------------
...
\begin{itemize}
  \item 推广到多个驱动、或直接驱动 transmon，在形式上是直接的（更换相应算符与端口耦合）。
\end{itemize}

% --------------------------------------------------
\subsubsection{驱动产生相空间位移：相干态与位移算符}
% --------------------------------------------------

$\hat H_d$ 本质上是谐振子相空间中的\textbf{位移（displacement）}生成元。适当选择驱动参数可将谐振子态从真空制备到任意相干态（Carmichael 2002; Gardiner \& Zoller 1999）：
...
% --------------------------------------------------
\subsubsection{近似来源与适用性：驱动项来自 RWA，大振幅时会偏离}
% --------------------------------------------------
...
对多数常见实验参数足够好，但当驱动幅度很大时，可能观察到偏离式(84)预测的效应（Pietik\"ainen et al.\ 2017; Verney et al.\ 2019）。

\subsection{附：Transmon qubit中的噪声}
Transmon qubit系统的噪声通常通过某个外参量$\lambda$的涨落进入系统：
\begin{itemize}
  \item 电荷/电压噪声
  \item 磁通噪声
  \item 临界电流噪声
...
\end{itemize}
\textbf{噪声谱密度：}
$$S(\omega) = \int_{-\infty}^{\infty} dt \, e^{i\omega t} \langle \delta\lambda(t) \delta\lambda(0) \rangle$$

将耦合算法投影到qubit子空间中，得到$$A = A_{\perp}\sigma_x + A_{\parallel}\sigma_z + A_0 I$$
\begin{itemize}
  \item $A_{\perp}$：导致能量弛豫的横向耦合
  \item $A_{\parallel}$：导致退相干的纵向耦合
\end{itemize}
\subsubsection{$T_1,T_2,T_\phi$}
\paragraph{$T_1$}
描述从激发态到基态的能量弛豫过程，通常由高频噪声驱动($\omega_{01}$附近)，速率可以写为耦合强度与噪声谱在qubit处频率的乘积：
\paragraph{$T_2$和$T_{\phi}$}
$T_2$描述相干态的寿命，包含能量弛豫和纯退相干两部分
$T_{\phi}$为纯退相干时间，通常由低频噪声驱动(接近0频率)，速率可写为灵敏度与噪声谱在0频率处的乘积。

\subsubsection{主要噪声机制}
\paragraph{电磁环境耦合与辐射损耗}
\begin{itemize}
  \item 阻抗型损耗：量子比特通过电容/电感与环境耦合，环境的阻抗导致能量泄漏
  \item Purcell衰减：表现为腔的线宽会在qubit频率附近打开衰减通道
\end{itemize}
\paragraph{材料缺陷与界面态}
\paragraph{准粒子}
\paragraph{磁通噪声}
\paragraph{电荷噪声和临界电流噪声}
\paragraph{光子噪声与测量噪声}

\subsubsection{实验上的测量}
\paragraph{$T_1$}
拟合指数衰减曲线
\subparagraph{理论推导：}

\paragraph{$T_2^*$}
Ramsey干涉：$$p_e(\tau) = \frac{1}{2}(1 + e^{-(\tau/T_2)^*}\cos \Delta \tau)$$
$\alpha$的值取决于低频噪声
\subparagraph{理论推导：}
考虑Markov主方程：$$\dot{\rho} = \frac{1}{i\hbar}[H,\rho] + \gamma_1 \mathcal{D}(\sigma_-)\rho + 2\gamma_{\phi} \mathcal{D}(\frac{\sigma_z}{2})$$
取相干项，有$$\dot{\rho_{01}} = (i\delta -\frac{\gamma_1}{2} - \gamma_{\phi})\rho_{01}$$
则有$$ \rho_{01} = \rho_{01}(0)e^{i\delta}e^{-\tau/T_2}$$
以上结果为考虑Markov白噪声的结果，实验中涉及一些更特定的噪声
\subparagraph{实验情形}
实验中，qubit的频率会发生漂移$$\omega_q' = \omega_q + \Delta \omega$$
因此在多次重复Ramsey干涉得到的曲线包络包含平均项$\braket{e^{-i\Delta \omega \tau}}$，其会导致测得的$T_2^*$小于实际的$T_2$，越小，说明低频噪声/漂移占主导。
\paragraph{$T_{2}^{echo}$}
施加脉冲序列$$\pi/2 - \tau - \pi - \tau - \pi/2$$
通过施加相位反转序列使得慢漂移在前后演化积累的相位抵消。

\subsection{附：相干态}
相干态是微波谐振腔模式中最接近经典的量子态：
\begin{itemize}
  \item 波包中心位置遵循经典谐振子运动方程，运动时波包形状不变
  \item 相干态的涨落与真空态相同，接近海森堡不确定关系的最小值
\end{itemize}
在qubit中，相干态可以：
\begin{itemize}
  \item 作为读出的指针态，腔会根据qubit的状态进入不同的相干态
  \item 玻色编码的构件：cat code ，GKP code
  \item 相空间操作的参考态：相干态在相空间中的图像以及变换直观
\end{itemize}
\section{V. CIRCUIT QED 中的测量（Measurements in circuit
...
\subsection{A. Microwave field
detection（微波场探测）}\label{a.-microwave-field-detectionux5faeux6ce2ux573aux63a2ux6d4b}

\subsubsection{A1. 典型测量链路（图
14）}\label{a1.-ux5178ux578bux6d4bux91cfux94feux8defux56fe-14}

\textbf{链路结构（从信号源到数字处理）}

\begin{itemize}
   \item 室温微波源产生 RF 信号 → 经多级衰减器（Att.，4K级，still级，CP级）进入低温腔输入端（谐振器），与qubit发生作用，随后将携带qubit信息的信号从谐振器输出。
   \begin{itemize} \item 衰减器作用：吸收从室温沿线路向样品传播的\textbf{热噪声}（thermal
noise），降低信号功率，并实现热锚定（不同温区分段衰减）。 \end{itemize}
  \item 从谐振器输出的信号非常微弱，需要进行放大处理。腔输出场先经过\textbf{环行器（circulator）}再进入放大链。首先通过接近量子极限的低温放大（JPA/TWPA等，在进入HEMT放大。
     \begin{itemize}
      \item 环行器：只允许信号正向传输，强烈抑制反向传播（来自放大器的噪声回灌到腔/样品）。 工程限制：环行器常依赖永磁体，体积大、难片上集成，还引入插入损耗与片外损耗；目前有研发超导片上环行器的努力。
      \item 量子极限放大器，位于10mK温度区，可以引入最低理论极限的放大器。常见的量子极限放大器有：JPA：噪声低，但带宽窄，只能放大特定频率范围的信号，噪声达到量子极限；TWPA：将多个Josephson结串联，可以同时放大不同频率的信号，噪声接近量子极限。
      \item HEMT：位于4K温度区，增益高但噪声较大，常用来进一步放大信号。
     \end{itemize}
  \item 随后进入室温电子学系统。首先与参考信号LO混频，从高频变频到中低频IF信号，随后信号经过ADC采样，再由 FPGA实时处理/记录。
\end{itemize}
\begin{figure}[htbp]
  \centering 
  \includegraphics[width = 0.6\textwidth]{img/microwave measurement.png}
  \caption{典型测量链路}
\end{figure}
\subsubsection{A2. 有限带宽下的``滤波输出场''（式
...
\subsubsection{A3. 放大：相位保持放大器的输入输出关系（式
...
\subsubsection{A4. 含衰减与多级放大的总附加噪声（图 15 与式
...
\subsubsection{A5. 量子效率的另一种等效定义（图 15(b)，式
...
\begin{figure}[htbp]
  \centering 
  \includegraphics[width = 0.8\textwidth]{img/quantum efficiency.png}
  \caption{量子效率的等效定义}
\end{figure}

\subsubsection{A6. 放大后的电压表达式（式
...
\subsubsection{A7. IQ 混频与外差检测（图 16，式
...
\item 目的：降低RF频率，从而使得ADC和数字处理更可行；同时获得信号的两正交分量（I/Q），以完整表征信号的幅度与相位。
\item
  IQ mixer 结构要点（图 16）：
...
\begin{figure}[htbp]
  \centering 
  \includegraphics[width = 0.8\textwidth]{img/IQ mixer.png}
  \caption{IQ 混频与外差检测}
\end{figure}
...
\subsubsection{A8. 场正交分量与旋转参考系（式
...
\subsubsection{A9. 同相（homodyne）检测作为特例（式
...
\subsection{B. Phase-space representations and their relation to
field
...
\subsubsection{B2. Wigner 函数与特征函数（式
...
\subsubsection{B4. 相干态的 Wigner 函数（图 17，式
...
\subsubsection{B5. Wigner 边缘分布与正交分量测量（式
...
\subsubsection{B6. Husimi-Q 分布与异频（heterodyne）检测（式
...
\item 由于$H_{disp}$与测量算符$\sigma_z$对易，因此测量是QND的。
\end{itemize}

\paragraph{C1.2 腔内相干幅度的动力学（式 (109a)(109b)，图
18）}\label{c1.2-ux8154ux5185ux76f8ux5e72ux5e45ux5ea6ux7684ux52a8ux529bux5b66ux5f0f-109a109bux56fe-18}
qubit的测量要求在尽可能短的时间达到足够高的保真度，下面分析腔内场的动力学以理解指针态的形成与分离过程。
\begin{itemize}
\tightlist
\item
  在 Langevin 方程框架下（文中引用先前式 (75)），腔内复振幅
  \(\alpha_\sigma(t) = Ae^{i\phi}\)（\(\sigma=g,e\)）满足（式 (109)）： \[
  ...
  为测量驱动相对``裸腔频率''的失谐（旋转项），\(\kappa\)
  为腔能量衰减率（耗散项），\(\varepsilon(t)\) 为驱动项，表示外部输入的试探微波脉冲幅度，用于探测腔的频率。
\item
...
\begin{figure}[htbp]
  \centering 
  \includegraphics[width = 0.6\textwidth]{img/trajectory in phase space.png}
  \caption{稳态腔内场的动力学}
\end{figure}
\paragraph{C1.3 稳态解与稳态正交分量（式
...
\begin{figure}[htbp]
  \centering 
  \includegraphics[width = 0.8\textwidth]{img/coupler shift.png}
  \caption{稳态解与稳态正交分量}
\end{figure}
\paragraph{C1.4
...
\paragraph{C2.3 测量保真度（式 (116)）与与 SNR
...
\paragraph{C2.4 长时间极限的 SNR 标度与参数优化（式 (117)
...
\paragraph{C2.5 测量光子数的上限：色散近似失效与非
...
\paragraph{C2.7 实验水平与放大器的重要性；量子效率与退相干的联系（式
...
\paragraph{C3.2 高功率读出与量子比特 ``punch
...
\paragraph{C3.3 Squeezing（压缩）以提升 SNR
...
\paragraph{C3.4 Longitudinal readout（纵向读出）与合成纵向耦合（式
...
\subsection{A. Resonant regime（共振区）}

\subsubsection{A.1 Jaynes--Cummings 模型在共振时的低能本征态（dressed states）}
...
\subsubsection{A.4 低激发功率极限：截断希尔伯特空间与稳态透射}
...
\subsubsection{A.5 三种共振耦合区的区分与物理图像}

\paragraph{A.5.1 Bad-cavity limit（坏腔极限）}
\subparagraph{定义与条件}
...
\subparagraph{简化模型与 Purcell 衰减（共振形式）}
...
\subparagraph{与色散区 Purcell 公式的关系（文中强调“不矛盾”）}
...
\subparagraph{频谱响应特征（与图示对应的现象总结）}
...
\paragraph{A.5.2 Bad-qubit limit（坏比特极限）}
\subparagraph{定义与条件}
...
\subparagraph{“反 Purcell”光子衰减率（inverse Purcell）与谱线展宽}
...
\subparagraph{历史与电路 QED 中的实现要点（文中举例）}
\begin{itemize}
...
\subparagraph{二维谱图与避免交叉（avoided crossing）}
...
\subparagraph{时域证据：真空拉比振荡（Vacuum Rabi Oscillations）}
...
\subparagraph{关键澄清：弱激发下“像两经典耦合振子”，避免交叉并非量子性的充分证据}
...
\subparagraph{量子性更清晰的证据：Jaynes--Cummings 梯子的 $\sqrt{n}$ 非简谐（anharmonicity）}
...
\subparagraph{多光子跃迁与跃迁选择性（Fig.23 的要点）}
文中（对应 Fig.23）总结：
\begin{itemize}
...
\subparagraph{高功率/高温下的量子--经典过渡（quantum-to-classical transition）}
...
\subsection{B. Dispersive regime（色散区）}
...
\subsubsection{B.1 色散哈密顿量的“重写”：同时看见 cavity pull 与 qubit pull}
...
\subsubsection{B.2 Qubit spectroscopy（量子比特谱学）：先假设腔在真空}

\paragraph{B.2.1 基本测量思路：pump + probe 与“量子跳跃谱学”}
...
\paragraph{B.2.2 两能级 Bloch 方程稳态：洛伦兹线型与功率展宽}
...
\paragraph{B.2.3 通过腔读出谱线：弱色散下相位与布居近似成正比}
...
\subsubsection{B.3 ac-Stark shift 与测量导致的展宽/退相干}

\paragraph{B.3.1 平均光子数导致 qubit 频移：用频移反推腔内光子数（需谨慎）}
...
\paragraph{B.3.2 光子数涨落导致测量退相干：photon shot noise / measurement-induced dephasing}
...
\subsubsection{B.4 强色散极限（strong dispersive limit）：光子数分辨谱与非线性效应}

\paragraph{B.4.1 光子数分辨（photon-number splitting）}
...
\paragraph{B.4.2 用 ac-Stark 或线宽反推 $\bar n$ 的“适用边界”：高功率下 cavity pull 变小}
...
\subsection{C. Beyond strong coupling: ultrastrong coupling regime（超强耦合区）}
...
\subsubsection{C.1 从 Jaynes--Cummings 到量子 Rabi 模型：激发数不再守恒}
...
\subsubsection{C.2 为什么电路平台更容易进入 USC：有效“精细结构常数”可工程化}
...
\paragraph{C.2.1 实现路径 1：in-line transmon（“在线”transmon）}
...
\paragraph{C.2.2 实现路径 2：与 flux qubit 的电流式（galvanic）强耦合与高阻抗工程}
...
\subsubsection{C.3 USC 的直接物理后果（文献强调的三点）}

\paragraph{C.3.1 Bloch--Siegert shift（布洛赫--西格特频移）}
...
\paragraph{C.3.2 基态不再是 $\ket{g,0}$：真空被“缀饰”，出现虚光子成分}
...
\paragraph{C.3.3 耗散与主方程需要重新写：不能直接套用弱耦合/JC 下的 Lindblad 形式}
...
\noindent\textbf{为何 circuit QED 适合量子计算}
...
\begin{itemize}
  ...
  \item 实际芯片常将多种功能（驱动线、通量线、读出腔、耦合腔、Purcell 滤波器等）集成在同一封装/芯片上。
\end{itemize}
...
\subsection{A.\quad 单比特控制（Single-qubit control）}

\subsubsection{两种常见驱动路径}
...
\subsubsection{驱动幅度与输入场的关系（输入输出观点）}
...
\subsubsection{含驱动的 transmon 模型：实验室系与旋转系}
...
\subsubsection{截断到两能级：Bloch 球上的旋转轴由相位决定}
...
\subsubsection{脉冲整形与 DRAG 思路}

\noindent\textbf{为什么不能简单用方波？}
...
\begin{itemize}
  \item 用更平滑的脉冲，例如高斯脉冲，以减少高频谱成分，但门时长可能变长；
...
\paragraph{DRAG}
DRAG的核心思想是：在主驱动上施加一个平滑的包络，同时在与其正交方向施加一个与主包络时间导数成比例的辅助驱动，以抵消非简谐引起的泄漏和相位误差。
\par 例如，在I通道的主驱动为$\Omega_x(t)$，则在Q通道施加一个辅助驱动$\Omega_y(t) = -\beta \dot{\Omega}_x(t)$，其中$\beta$是一个实验校准的参数。
\subparagraph{DRAG的标定}

DRAG的标定包括
\begin{itemize}
  \item 主脉冲幅度：决定旋转角度
  \item 脉冲宽度：决定门事件和频谱宽度
  \item $\beta$ 参数：调整辅助驱动的强度以最小化泄漏和相位误差
  \item 驱动频率
\end{itemize}

为了实现更高的门保真度，还有二阶DRAG等更复杂的方案，涉及更多参数和更精细的优化。

\paragraph{GRAPE}
GRAPE可以自动寻找一组时间依赖的控制脉冲，以最大化某个目标函数（例如门保真度）。它通过计算系统动力学的梯度来优化控制参数，适用于复杂系统和多参数优化。

其可以实现多种任务的优化，如量子门，制备特定量子态等。

\subparagraph{基本原理}

GRAPE进行优化时，需要有一个代价函数，例如优化量子门时的代价函数就是门保真度，态制备时的代价函数就是制备态与目标态的重叠度。

在实际计算时，首先将总时间切片为N份，脉冲函数变为一个N维向量，每个分量对应一个时刻的哈密顿量，由此可以计算每个时刻的传播算符，各项相乘可以得到总传播算符，即模拟门算符，计算代价函数，并通过反向传播计算梯度，最后使用梯度下降等方法更新控制参数。
\subsubsection{$Z$ 轴旋转的实现：真实 $Z$ 与虚拟 $Z$}

\noindent\textbf{方式 1：调节量子比特频率（真实 $Z$）}

\begin{itemize}
  \item 在无驱动时（$\Omega_R=0$），通过暂时改变 $\omega_q$ 并持续一段时间，使态矢量积累相位，得到绕 $Z$ 轴旋转；
  \item 常用手段：通量可调 transmon 施加磁通脉冲；
  \item 代价：引入对通量噪声的敏感性，带来更多退相干通道。
\end{itemize}

\noindent\textbf{方式 2：失谐微波驱动诱导 ac-Stark shift（有效 $Z$）}
...
\begin{equation}
  ...
  \label{eq:acstark}
\end{equation}
因此通过驱动幅度或持续时间即可控制绕 $Z$ 轴的相位积累。

\noindent\textbf{方式 3：虚拟 $Z$（Virtual $Z$ gate）}
...
\subsection{B.\quad 双比特门（Two-qubit gates）}

\subsubsection{难点与设计目标}

\begin{itemize}
  \item 双比特门通常比单比特门更难，误差率也更高；
  \item 核心挑战是需要\textbf{快速、可控地打开和关闭}比特--比特相互作用；
  \item 与单比特门“打开/关闭微波驱动”相比，双比特门的 on/off 比实现更困难。
\end{itemize}

\subsubsection{方案分类（按相互作用激活方式）}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{img/two qubit gate.png}
  \caption{双比特门方案分类示意图（文献图示）}
  \label{fig:2qgate}
\end{figure}
\subsubsection{B.1\quad 交换型相互作用（Exchange-type interactions）}

\noindent\textbf{(a) 直接电容耦合（Direct capacitive coupling）}

两 transmon 直接通过电容耦合时，常用模型为
\begin{equation}
...
其中
\[
  \hat H_{qi}=\hbar\omega_{qi}\hat b_i^\dagger \hat b_i-\frac{E_{C_i}}{2}(\hat b_i^\dagger)^2\hat b_i^2.
\]
...
\noindent\textbf{两能级近似与 $\sqrt{i\mathrm{SWAP}}$}

当两比特调到共振 $\omega_{q1}=\omega_{q2}$ 并在旋转波近似下，式 \eqref{eq:directcap} 在两能级子空间约化为
\begin{equation}
...
\noindent\textbf{如何实现 on/off：失谐开关与残余相互作用}

\begin{itemize}
  \item 常见做法是通过将两比特失谐 $\Delta_{12}=\omega_{q1}-\omega_{q2}$ 来“关闭”交换作用。
  \item 当 $J/\Delta_{12}\ll 1$ 时，交换项被抑制，但会出现二阶残余耦合，常表现为 \textbf{$ZZ$} 形式，量级可写成
  \[
    \propto \frac{J^2}{\Delta_{12}}\,\sigma_{z1}\sigma_{z2},
  \]
  其导致条件相位累积，进而带来门误差，需要用校准或回波技术补偿。
  \item 若想进一步提高 on/off 比，可引入\textbf{可调耦合器（tunable coupler）}，使 $J$ 本身可调，但电路复杂度与潜在退相干通道也会增加。
\end{itemize}

\noindent\textbf{(b) 谐振腔介导耦合（Resonator-mediated coupling）}

\noindent\textbf{基本模型：}若两个量子比特通过同一谐振腔模式作为量子总线耦合，而彼此无直接耦合，则
\begin{equation}
...
\noindent\textbf{两种用法：}
\begin{itemize}
  \item \textbf{真实激发（真空 Rabi 交换）思路：}先将比特 1 与腔调到共振，把激发交换到腔，再把腔激发交换给比特 2，可实现态交换并演示“总线”作用，但该序列本身并不直接对应一个标准的纠缠双比特门。
  \item \textbf{色散区（dispersive）虚激发思路：}两比特均与腔大失谐，仅虚占据腔模，通过二阶过程产生有效比特--比特相互作用，更常用于门实现与可控耦合分析。
\end{itemize}

\noindent\textbf{色散变换后的有效哈密顿量结构：}令 $\Delta_i=\omega_{qi}-\omega_r$，在色散近似下，保留到二阶，可得到有效形式
\begin{align}
...
\begin{itemize}
  ...
  \item $\Xi_{ij}$ 对应有效的比特--比特非线性或条件频移，常与 $ZZ$ 相关。
\end{itemize}
在常见参数区间里，$\Xi$ 往往比直接交换项 $J$ 小，因此在某些讨论中可近似忽略。

\noindent\textbf{总线诱导的有效交换耦合强度：}
\begin{equation}
...
FIG.30图展示了两个 transmon 量子比特耦合到同一个谐振腔时的能谱图（能量频率随调谐参数变化的曲线）。

图中的状态标记为 $|q_1, q_2, r\rangle$，分别代表 |量子比特1的激发数, 量子比特2的激发数, 谐振腔的光子数>。

浅蓝色虚线 (Guides to the eye)： 代表如果不发生任何相互作用时，量子比特原本的频率（裸频率 Bare frequency）。
深蓝色实线： 代表实际发生相互作用后，系统真实的能量状态。当两条线本该交叉时，如果它们之间有耦合，就会发生“能级排斥” (Avoided crossing / Anticrossing)，即两条线会避开彼此，产生一个间隙。


\textbf{图 (b) -- 单激发流形 (1-excitation manifold)}
这部分只包含系统中总共只有1个能量量子的情况。

\begin{itemize}
    \item \textbf{横轴：} 正在被调节频率的量子比特（假设为量子比特 2）。
    \item \textbf{纵轴：} 系统的频率/能量 (GHz)。
    \item \textbf{水平的直线 ($\ket{1, 0, 0}$)：} 代表频率固定的量子比特 1处于激发态。它的能量不随横轴变化。
    \item \textbf{倾斜向上走的线 ($\ket{0, 1, 0}$)：} 代表正在被调频的量子比特 2处于激发态。随着横轴向右，它的能量越来越高。
    \item \textbf{现象 1 -- 真空拉比分裂 (Vacuum Rabi splitting, 标有 $2g_\downarrow$)：}
          当倾斜的线在较低频率处（图的偏左侧，虽然腔的水平线没画出，但能看出在这里发生了弯曲）遇到谐振腔的频率时，量子比特 2 与谐振腔发生共振。这里产生了一个较大的能级排斥，间隙大小为 \textbf{$2g$}（量子比特与腔的直接耦合强度）。
    \item \textbf{现象 2 -- 腔介导的耦合 (标有 $2J_\downarrow$)：}
          这是这张图要表达的\textbf{核心}！当倾斜的线继续往上走，在频率达到 \(\sim 8\,\text{GHz}\) 时，它遇到了固定频率的量子比特 1 ($\ket{1, 0, 0}$)。
          \textbf{注意：} 虽然这两个量子比特没有直接物理连接，而且此时它们的频率都远离谐振腔的频率，但由于谐振腔的“虚拟介导”作用，它们在这里依然发生了一个较小的能级排斥。这个间隙的大小就是前面提到的\textbf{有效耦合强度 $2J$}。这就直观地证明了“量子总线”的存在。
\end{itemize}

\textbf{图 (a) -- 双激发流形 (2-excitation manifold)}
这部分包含系统中总共有2个能量量子的高阶情况，用来展示系统更复杂的动力学。

\begin{itemize}
    \item \textbf{水平线 ($\ket{2, 0, 0}$)：} 固定频率的量子比特 1 吸掉了 2 个能量量子。能量固定。
    \item \textbf{平缓倾斜的线 ($\ket{1, 1, 0}$)：} 两个量子比特各吸掉 1 个能量量子。因为其中一个是固定的，另一个是变化的，所以斜率是下一条线的一半。
    \item \textbf{陡峭倾斜的线 ($\ket{0, 2, 0}$)：} 被调频的量子比特 2 独自吸掉了 2 个能量量子。能量随调节快速上升。
    \item \textbf{现象 -- 高阶相互作用 (标有 $\zeta_\downarrow$)：}
          当$\ket{1,1,0}$，能量调节至与$\ket{2,0,0}$一致时，发生了能级排斥（间隙为 $\zeta$）。这表明在多激发状态下，通过谐振腔介导的相互作用同样会引起复杂的能级混合，这对于设计高保真度的量子门（需要避免这些高能级的泄漏）是非常重要的参考数据。
\end{itemize}

\textbf{总结图意}
图 (b) 完美地用实验/数值模拟数据证明了文本的理论：\textbf{你不需要让两个量子比特直接接触，只要它们都连在同一个谐振腔上，当它们的频率靠近时（图b中的 $2J$ 处），它们就能自动“感知”到对方并发生物理耦合。}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{img/spectrum of two qubit.png}
  \caption{色散总线介导的交换耦合示意图（文献图示）}
  \label{fig:busanticrossing}
\end{figure}
\noindent\textbf{真空腔近似下的简化理解：}若腔初态为真空且在色散相互作用下仅被\textbf{虚占据}，则对实际动力学而言，式 \eqref{eq:Heffbus} 中与 $\hat a^\dagger\hat a$ 成正比的项在很多情形下可忽略；此时可将问题近似回到“有效直接交换耦合”的图像。

\noindent\textbf{如何用总线实现门：}
\begin{itemize}
  \item 类似直接耦合，通过将两比特\textbf{调入或调出}彼此共振，但仍可处于与腔色散区，来开启和关闭交换；
  \item 或者使 $g_i$ 可调，通过可调耦合器或可调耦合结构实现更好的开关比与频谱可管理性。
\end{itemize}

\noindent\textbf{色散总线在时间域的简化理解：}
\begin{itemize}
  ...
  \item 因此在有效哈密顿量中，若腔始终近似处于真空，可在实践上忽略与 $\hat a^\dagger\hat a$ 成正比的那一类项；动力学主要退化为\textbf{有效直接交换耦合}形式。
  \item 结论是：当两比特彼此调到近共振、但仍与腔保持色散失谐时，腔作为量子总线可提供等效的交换相互作用，从而以与直接电容耦合类似的方式实现纠缠门。
\end{itemize}

\subsubsection{B.2\quad Flux-tuned $|11\rangle$--$|02\rangle$ phase gate（通量调谐的 $|11\rangle$--$|02\rangle$ 相位门）}

\noindent\textbf{门的物理来源与适用对象}

\begin{itemize}
  \item 该门是一类\textbf{受控相位门（controlled-phase gate）}，特别适用于弱非简谐的 transmon。
  \item 它利用两比特交换耦合，耦合可来自直接耦合或谐振腔总线介导，导致\textbf{非计算能级参与}：计算态 $|11\rangle$ 与非计算态 $|02\rangle$ 在调谐过程中产生\textbf{反交叉}。
  \item 与 $\sqrt{i\mathrm{SWAP}}$ 不同，11--02 相位门不需要将\emph{计算态之间}调到共振，而是通过接近 $|11\rangle$--$|02\rangle$ 反交叉来使 $|11\rangle$ 获得额外的能级排斥与相位积累。
  \item 在做门时，使系统缓慢地接近能级简并点，于是由于能级排斥，只有$\ket{11}$会发生能级偏移，导致其能量比$E_1 + E_2$低$\zeta$，由于相位累积取决于能量，$\ket{11}$会比单比特情形积累不同的相位，因此在除去全局相位以及单比特相位（virtual Z）后，即得到CZ。
\end{itemize}

\noindent\textbf{绝热通量脉冲下的幺正以及与 $\mathrm{CZ}$ 的关系}
...
该门在忽略单比特相位，可通过虚拟 $Z$ 或后处理补偿后，等价于标准受控相位门：
\begin{align}
  \hat C_Z(\phi)
...
\noindent\textbf{非绝热（sudden）实现思路}

\begin{itemize}
  \item 除了绝热进出反交叉外，还可将系统\textbf{快速脉冲}到反交叉附近并停留时间 $t$。
  \item 在“突然近似”下，初态 $|11\rangle$ 在 $\{|11\rangle,|02\rangle\}$ 子空间内发生两能级演化，哈密顿量可以写为$$\begin{pmatrix} E_{11} & \frac{\zeta}{2} \\ -\frac{\zeta}{2} & E_{02} \end{pmatrix}$$，其中$E_{11}$和$E_{02}$的能量相等，此时的系统会发生一个Rabi振荡：
  \[
    |11\rangle \rightarrow \cos(\tfrac{\zeta t}{2\hbar})|11\rangle + i\sin(\tfrac{\zeta t}{2\hbar})|02\rangle,
  \]
  其中 $\zeta$ 表示反交叉处的能级排斥，也就是能隙。
  \item 选择 $t=h/\zeta$ 时，$|11\rangle$ 回到自身但获得一个负号，即相位 $\pi$，而其它逻辑态因远离共振近似只积累平庸相位，从而实现 $\mathrm{CPHASE}$。
\end{itemize}

\noindent\textbf{工程挑战：通量脉冲畸变与补偿}

\begin{itemize}
  \item 控制电子学与走线有限带宽会导致通量脉冲\textbf{波形畸变}；并且通量响应可能具有\textbf{历史依赖}，从而造成门误差与漂移。
  \item 常用缓解方法包括：对已知传递函数做\textbf{预失真（pre-distortion）}；以及围绕 transmon 甜点设计对称通量轨迹以抵消不期望项。
\end{itemize}

\subsubsection{B.3\quad All-microwave gates（全微波双比特门）}

\noindent\textbf{动机}

\begin{itemize}
  \item 前述基于调频或调耦合的门需要较大的频率调谐范围或可调耦合器，可能带来额外退相干与频谱拥挤问题。
  \item 全微波门希望像单比特门一样，主要通过\textbf{打开和关闭微波驱动}来实现双比特纠缠，在固定频率比特架构中特别有吸引力。
\end{itemize}

本小节聚焦两类代表性方案：\textbf{RIP} 与 \textbf{CR}。

\noindent\textbf{(a) Resonator-induced phase (RIP) gate}

\noindent\textbf{基本设置与哈密顿量}

\begin{itemize}
  \item 两个彼此强失谐的量子比特，可近似忽略其直接交换 $J$，共同\textbf{色散耦合}到同一个谐振腔模式。
  \item 通过驱动谐振腔，在计算子空间中产生条件相位，也就是有效 $ZZ$。
\end{itemize}
...
\noindent\textbf{有效哈密顿量与条件相位来源}

通过转到驱动旋转系并做极化子变换，可得到近似有效形式
\begin{align}
  \frac{\hat H''}{\hbar}
  &\simeq
...
\begin{itemize}
  ...
  \item 通过\textbf{绝热}地开关 $\epsilon(t)$ 并使腔在门始末回到真空或相同场态，可避免门结束时腔与比特残余纠缠。
\end{itemize}

\noindent\textbf{限制与改进方向}

\begin{itemize}
  \item 为避免测量诱导退相干，通常需要较强腔失谐，即 $\delta_r\gg\kappa$，这会压低有效相互作用强度，导致门时间变长。
  \item 可通过脉冲整形、在输出场上使用压缩态降低可区分信息、或采用纵向耦合等思路缓解。
\end{itemize}

\noindent\textbf{(b) Cross-resonance (CR) gate}

\noindent\textbf{核心思想}

\begin{itemize}
  \item 两比特彼此失谐但存在交换耦合 $J$，该耦合可直接来自电容耦合，也可由总线介导。
  \item 直接驱动比特 1，也就是控制比特，驱动频率选在比特 2，也就是目标比特的频率附近，从而在目标比特上产生依赖于控制比特态的有效驱动项，典型为 $ZX$。
\end{itemize}

\noindent\textbf{两能级近似下的出发点}

无驱动时，在两能级近似下，耦合系统可写为
\begin{equation}
  \hat H
  =
  ...
  \tag{143}
\end{equation}
在 $|J/\Delta_{12}|\ll 1$，其中 $\Delta_{12}=\omega_{q1}-\omega_{q2}$，下做对角化或 Schrieffer--Wolff 变换后，驱动比特 1 的项会“泄露”到比特 2 并带上条件性，从而出现有效的 $ZX$ 相互作用。

\noindent\textbf{驱动诱导的条件项（直观近似）}
...
\begin{align}
  ...
  \tag{144}
\end{align}
并在选择 $\omega_d\approx \tilde\omega_{q2}$ 时，$\sigma_{z1}\sigma_{x2}$ 项可用于构造 CNOT，配合单比特项回波消除即可。

\noindent\textbf{考虑 transmon 多能级后的更准确模型}
...
\begin{equation}
  ...
  \tag{145}
\end{equation}
经过旋转系与 Schrieffer--Wolff 变换，并截断到两能级后，可得到
\begin{align}
  \hat H'
  \simeq\;
...
\noindent\textbf{重要结论：为何 CR 难，且需要校准与回波}

\begin{itemize}
  \item \textbf{有用项：}驱动产生的 $\sigma_{z1}\sigma_{x2}$，也就是 $ZX$，实现纠缠。
  \item \textbf{不想要的项：}
    \begin{itemize}
      \item 固有的 $\sigma_{z1}\sigma_{z2}$，也就是 $ZZ$，项 $\propto \chi_{12}$ 会使目标比特频率依赖控制比特态，导致驱动无法同时对两种控制态都严格共振，从而限制门速度与保真度；
      \item 存在对目标比特的串扰驱动，例如 $-\frac{J}{\Delta_{12}}\sigma_{x2}$，以及更复杂的多能级效应导致的偏差。
    \end{itemize}
  \item 实验上通常采用\textbf{CR-echo} 等回波序列抵消单比特项与部分误差项，并通过标定提取有效 $ZX$ 强度与相位。
  \item 门时长历史上多在数百 ns 量级；通过更精细的建模与校准可推进到更短时间尺度，具体依平台而定。
\end{itemize}

\subsubsection{B.4\quad Parametric gates（参数调制门）}

\noindent\textbf{基本思想}

\begin{itemize}
  \item 通过对系统参数做时间周期调制，例如耦合强度、比特频率、总线谐振腔频率等，在频域产生边带，从而把原本失谐的相互作用“搬移”到共振条件，实现可控的有效耦合。
  \item 目标是获得较大 on/off 比，并缓解频谱拥挤；调制频率常在\textbf{几百 MHz}量级，相较直接微波驱动门的 GHz 频段更易工程实现。
\end{itemize}

\noindent\textbf{B.4.1\quad 调制耦合强度 $J(t)$：恢复交换相互作用（类 iSWAP）}
...
\begin{equation}
  ...
  \tag{149}
\end{equation}
当 $J(t)=J_0$ 为常数且 $|J_0/(\omega_{q1}-\omega_{q2})|\ll 1$ 时，上式快速振荡项平均掉，门处于“off”状态。选择调制频率
\[
...
\noindent\textbf{B.4.2\quad 调制比特频率：FM 边带与贝塞尔函数权重}

另一种常见做法是调制某个比特频率，例如
\[
  \omega_{q1}(t)=\omega_{q1}+\epsilon\sin(\omega_m t),
\]
在频域产生边带并与另一比特对齐。一个典型有效表达可写为
\begin{equation}
  \hat U
  =\exp\!\left[
    -\frac{i}{\hbar}\int dt\,
...
\noindent\textbf{实现路径的扩展}

\begin{itemize}
  \item 除了调制比特频率或耦合强度，也可调制总线谐振腔频率，例如在传输线谐振腔中引入 SQUID 以实现可调电感，从而调谐腔频率，达到类似的参数门效果。
  \item 参数门的一个优势是可以在固定频率比特架构下实现较快门速并降低对通量噪声的敏感性，但具体收益仍取决于调制变量与工作点。
\end{itemize}

\subsubsection{图示内容的功能性解读}

\begin{itemize}
  \item 多层布线与模块，包括读出谐振腔、耦合谐振腔、控制线、电荷驱动、通量线、频率调谐与 Purcell 滤波器等，共同组成可扩展的 circuit QED 架构要素。
  \item 单比特门误差随脉冲长度或宽度变化的图展示：高斯脉冲与 DRAG 脉冲在短门时长下的误差差异；DRAG 通常可在更短门时长下保持较低误差，并与考虑多能级的数值模拟更一致。
  \item 双比特方案示意图总结了：直接耦合、耦合器或总线介导、全微波激活、参数调制等主要类别。
  \item 频谱反交叉图展示了：腔--比特真空 Rabi 分裂与比特--比特有效交换耦合造成的反交叉，可用于从谱线中提取 $g_i$ 与 $J$。
\end{itemize}
...
\begin{itemize}
  ...
  \item 但在超导平台中，\textbf{腔模或振子模}，尤其 3D 腔，往往具有更好的相干性，因此将量子信息编码到振子中可能更有利：
  \begin{itemize}
  ...
  \end{itemize}
  \item 关键点是：即使信息在振子中，仍通常需要一个\textbf{非线性元件}，如与腔色散耦合的 transmon 或其他约瑟夫森电路，来实现\textbf{制备、门操作、以及稳定子测量和纠错循环}；该非线性元件理想情况下只提供控制能力而尽量不引入额外噪声。
  \item 文献脉络上，振子编码在量子光学与 circuit QED 中对应多类方案，如 cat codes、binomial codes、GKP codes 等。
\end{itemize}

\subsubsection{C.2\quad 最简单的 binomial code 示例：抵抗单光子损失}

为理解玻色子码的基本思想，先考虑最简单的 binomial code：
\begin{equation}
...
\noindent\textbf{（1）平均光子数与“损失信息不可区分性”}
...
\begin{itemize}
  ...
  \]
  \item 因此一次光子损失事件发生的概率，在短时间内与 $\bar n$ 成正比，对两码字\textbf{相同}，观测到“发生了损失”并不会泄露逻辑信息。
  \item 这可视为纠错编码需要满足的一种“错误不携带逻辑信息”的条件，否则纠错测量本身会导致信息坍缩或退相干。
\end{itemize}

\noindent\textbf{（2）单光子损失作用下的可恢复性（系数保持）}
...
\noindent\textbf{（3）用光子数奇偶校验检测损失：单稳定子测量}
...
则
\begin{itemize}
  \item 无损失时处于偶奇偶，即 $\hat P=+1$；
  \item 一次损失后变为奇奇偶，即 $\hat P=-1$。
\end{itemize}
因此只需测量一个稳定子，也就是奇偶性，即可检测单光子损失事件；在 circuit QED 中，这类奇偶测量可高保真实现。

\noindent\textbf{（4）纠错循环与不可纠错错误概率的阶数改善}

若以时间步长 $\delta t$ 反复执行检测与纠错，则不可纠错错误的概率可从
\[
  O(\kappa\,\delta t)
  \quad \rightarrow \quad
  O((\kappa\,\delta t)^2),
\]
其中 $\kappa$ 是腔能量衰减率，也就是光子损失率。直观上，纠错把“一次损失”从致命错误变成可校正事件，主要不可纠错来源变为\textbf{同一轮间隔内发生两次损失}，因此概率阶数提高。

\noindent\textbf{对比：直接用 $|0\rangle,|1\rangle$ 存储信息的脆弱性}

若直接用 Fock 态 $\{|0\rangle,|1\rangle\}$ 存储，则单光子损失会将
\[
  c_0|0\rangle + c_1|1\rangle
  \xrightarrow{\hat a}
  c_1|0\rangle,
\]
逻辑信息不可逆丢失；这正说明了振子编码的关键收益。

\subsubsection{C.3\quad 与最简单四量子比特码的正面对比：开销与测量难度}

为了比较玻色子码与多物理量子比特码在“振幅阻尼”情形下的开销，可考虑一个能一阶纠正单比特振幅阻尼误差的四量子比特码：
\begin{align}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 最简单 binomial：只用到 $\{0,1,2,3,4\}$ 五个 Fock 态，有效维数约为 $5$。
  \end{itemize}
  \item \textbf{可纠正错误集合：}
  \begin{itemize}
    \item 四量子比特码可处理多类单比特误差：对应与无损失以及4个独立的单光子态的损失$\{I, \sigma_1^-,\sigma_2^-,\sigma_3^-,\sigma_4^-\}$；
    \item 最简单 binomial 码在此例中主要针对“无损失”和“单光子损失”两类事件进行检测与恢复。
  \end{itemize}
  \item \textbf{稳定子测量复杂度：}
  \begin{itemize}
    \item 稳定子定义为在编码态上本征值为1，在损失态上本征值为-1。
    \item 四量子比特码希尔伯特空间维数为16，为了编码01态，需要3个稳定子：$S_1 = Z_1Z_2$, $S_2 = Z_3Z_4$, $S_3 = X_-1X_2X_3X_4$
    \item binomial 示例只需测量\textbf{单个奇偶稳定子} $\hat P=(-1)^{\hat n}$，在 circuit QED 中相对更直接、高保真且对态破坏小。
  \end{itemize}
  \item \textbf{QEC:}
  \begin{itemize}
    \item 两者可以实现无误差以及一次损失的纠错，无法处理两阶以上的损失。
  \end{itemize}
\end{itemize}

\subsubsection{C.4\quad Cat-state bosonic encodings：四分量 cat code 与相空间图像}

除 binomial 外，circuit QED 中另一类广泛研究的玻色子编码是 \textbf{cat code}。一个实验中使用的四分量 cat 编码可写为
\begin{align}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{img/cat code.png}
  \caption{四分量 cat 码字在相空间中的 Wigner 函数示意图，展示了多峰结构与干涉条纹。}
  \label{fig:cat_code}
\end{figure}
\noindent\textbf{与 binomial 示例的关系：用 Fock 展开理解结构}

\begin{itemize}
  \item 将相干态关于Fock态的展开代入，可以看到$\ket{0_L}$只包含$\ket{4n}$项，$\ket{1_L}$只包含$\ket{4n+2}$项，这与binomial编码的情形类似。因此，该情形下的光子损失也可以纠正。
  \item 相空间表征，也就是 Wigner 函数，能直观显示 cat 码字的干涉条纹与多峰结构。cat code的良性质来源于其相空间的旋转对称性
\end{itemize}

\subsubsection{C.5\quad 两分量 cat code 与偏置噪声（biased noise）思想}
...
\noindent\textbf{关键物理：光子损失带来的“错误偏置”}

\begin{itemize}
  \item 在大振幅极限下，相干态满足 $\hat a|\alpha\rangle=\alpha|\alpha\rangle$，即一次光子损失主要带来幅度因子与相空间位移的“温和”变化，即：
  $$a\ket{0_L} \approx \ket{\alpha} \approx \ket{0_L} \,\, \, a\ket{1_L} \approx - \ket{-\alpha} \approx -\ket{1_L}$$
  ...
  \]
  \item 结果是：随着 $|\alpha|^2$ 增大，\textbf{比特翻转率指数下降}，而某些相位翻转通道仅随 $|\alpha|^2$ 线性增长，从而形成强烈的\textbf{噪声偏置}。
  \item 这启发了一种体系结构层面的策略：用玻色子码主要压制最“昂贵”的错误类型，再在上层纠错码中重点纠正剩余占主导的错误，例如以纠正相位翻转为主，从而降低容错量子计算的总开销。
\end{itemize}

\subsubsection{C.6\quad GKP 码的简述定位（本文此处不展开）}

\begin{itemize}
  \item 另一类已在 circuit QED 中实现演示的玻色子编码是 \textbf{Gottesman--Kitaev--Preskill (GKP) code}，其码字基于相空间中的\textbf{平移对称性（translational symmetry）}而非旋转对称性。
  \item 本文在此处仅指出 GKP 的实验进展与意义，不再详细展开。
\end{itemize}
