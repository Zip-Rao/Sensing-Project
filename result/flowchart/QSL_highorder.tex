...
\begin{tikzpicture}[
  ...
  extractbox/.style={line,rounded corners=3pt,align=center,minimum width=4.85cm,minimum height=2.25cm,font=\normalsize},
  kernelbox/.style={line,rounded corners=3pt,align=center,minimum width=3.7cm,minimum height=1.25cm,font=\normalsize},
...
\node[font=\bfseries\Large] at (19.6,3.35) {3. Update};
...
% Measurement bracket
\draw[line width=1pt]
  (4.75,1.80) -- (4.65,1.80) -- (4.65,-1.42) -- (4.75,-1.42);
\draw[line width=1pt]
  (9.45,1.80) -- (9.62,1.80) -- (9.62,-1.42) -- (9.45,-1.42);
\node[anchor=south west,font=\large] at (9.66,1.72) {$2N_{\mathrm{shot}}$};

% Entry and initialization
\node[smallblock,minimum width=1.45cm,font=\normalsize] (init) at (3.15,0.20) {Initialization};
\draw[arrow] (set.east) -- (init.west);
\draw[arrow] (init.east) -- (4.7,0.20);
\draw[line] (4.7,0.20) -- (5.0,0.20);

% Pulse sequence: Ry - R_{+-x}
\node[font=\normalsize] at (5.55,0.88) {$R_y(\alpha)$};
\begin{scope}[shift={(5.55,0.20)}]
  \draw[pulse,domain=-0.5:0.45,samples=100,smooth]
    plot (\x,{0.34*exp(-5*\x*\x)*sin(deg(28*\x))});
  \draw[envelope,domain=-0.5:0.48,samples=60,smooth]
...
% Pre-calibrated kernels: now G1 AND G3 (the high-order coefficients)
\node[kernelbox] (kernel) at (14.05,2.80)
  {pre-calibrated kernel\\[0.10cm]
   $G_{2m+1}=\textstyle\int\!\cdots\!\int\! k_{2m+1}\,d^{2m+1}t$\\[0.06cm]
  };

% High-order frequency extraction: cubic Newton instead of linear divide
\node[extractbox] (extract) at (14.2,0.18)
  {\textbf{High-order frequency extraction}\\[0.10cm]
   {\footnotesize \;\;$\displaystyle\Delta P_e=\sum_{m}\int\!\!\cdots\!\!\int k_{2m+1}\!\prod_a\delta\omega(t_a)\,d^{2m+1}t$}\\[0.02cm]
   $\big\downarrow$\,{\footnotesize $\delta\omega\approx\mathrm{const}$}\\[0.02cm]
   {\footnotesize \;\;$\displaystyle\Delta P_e=G_1\,\delta\omega+\tfrac{1}{6}G_3\,\delta\omega^{3}+\cdots$}\\[0.02cm]
   $\big\downarrow$\,{\footnotesize Newton}\\[0.02cm]
   $\widehat{\delta\omega}\;\Rightarrow\;\widehat{\Delta f}=\widehat{\delta\omega}/(2\pi)$};
\draw[arrow] (readout.east) -- node[above,font=\normalsize] {$\begin{aligned} &\Delta P_e \\  \, \, \, = &\frac{p_x-p_{-x}}{2}\end{aligned}$} (extract.west);

% Update
\node[updatebox] (update) at (19.6,0.20)
...
% Benefit note: high order extends the linear safe-zone (the fold)
\node[font=\footnotesize,align=center] at (14.2,-1.85)
