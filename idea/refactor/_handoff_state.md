...
| 执行者 (人/agent id) | refactor-phase-executor (P2 run) |
| 本次 token 实际消耗 | ~200K |
...
- [x] **P2** — 已实现协议 (case 0/1/2/4) 实验对象化 + KernelEstimator 去重 + src_mirror/protocal.py facade (2026-05-01, pending commit)
...
| 测试套件 | 上次结果 | 用时 |
|---|---|---|
| `pytest tests/unit -v` | 67 passed, 0 failed | 2.1s |
| `pytest tests/regression -m regression` | 4 passed, 0 failed | 24s |
| `pytest tests/equivalence` | 4 passed, 0 failed (cases 0/1/2/4) | 95s |
| `pytest tests/integration` | 6 passed, 0 failed | 95s |
| `pytest tests/ -v -m "not slow"` | 77 passed, 0 failed (4 deselected) | 115s |
...
2. **Qt GUI crash in regression tests** (pre-existing): `test_ramsey_default_baseline` triggers a Windows fatal exception from matplotlib's Qt backend because `src/protocal.py:65` calls `Phi.plot()`. The test still PASSES. This is a pre-existing production code issue. Not fixed in P2 (would require modifying src/).
...
4. **FluxSignal type=4 formula corrected**: P2 fixed a bug in `sqc/control/flux_signal.py` where the type=4 (asymmetric impulse) formula was incorrectly ported as simple subtraction instead of the double-exponential used by `src/signal.py`. This was a critical fix that enabled case 4 equivalence.

5. **PulseBase ABC fixed**: Removed `@property @abstractmethod` from PulseBase because instance attributes set in `__init__` don't satisfy property-based abstract methods. Changed to documentation-only base class.

6. **ALLXY experiment (§3.8)**: Skipped — marked as deferred in handoff. Not essential for P2.

7. **Case 5 (Cryoscope)**: Raises `NotImplementedError` in the facade. Will be internalized in P3c.

8. **Mirror layer**: `src_mirror/protocal.py` facade is complete. Delegates cases 0/1/2/4 to Experiment classes. All 4 cases produce identical numeric results to `src/protocal.py`.
...
|---|---|---|---|---|---|---|
| 2026-05-01 | P2 | refactor-phase-executor | (pending) | ✅ DONE | ~200K | 4 experiment classes; KernelEstimator; SlidingMeasurementRunner; IQReadoutModel; src_mirror/protocal.py facade; all 4 cases match old code 0/1/2/4; FluxSignal type=4 bug fixed; PulseBase ABC fixed; 77 total tests pass (67 unit + 4 regression + 4 equivalence + 6 integration) |
