...
|---|---|---|---|---|---|---|
| 2026-06-07 | P12.1 | Zip (主 session) | (pending) | ✅ DONE | ~200K | 瞬态测频 order≥3 修复 + Route B。诊断出**符号 bug**:order≥3 三次 Newton 用 G1_fit(Δ约定)与线性 G_freq(δω=−Δ约定)符号相反 → 返回 ω_d−Δ,误差≈−2Δ(比线性差)。修复:统一 δω=−Δ 约定(Route A 取 −G1_fit/−G3_taylor;Route B 用原始核积分)。(1) `_calibrate_g3_taylor` delta_max 默认→None=自适应(收缩至 G1 收敛;旧默认 0.08 使 G1 偏低0.6×);拆出 `_fit_g3_at_delta_max`;cache key 加 tag;`FrequencyMeasurement.g3_delta_max` 暴露。(2) Route B `_calibrate_g3_kernel_full` + `g3_source="kernel_full"`(∭k₃ dt³,依赖 P12 extract_off_diagonal)。(3) 移除 diag_legacy,未知 g3_source 早抛 ValueError。实测有效区 order3-fit 误差比线性↓~10×,符号正确;kernel_full 与 fit |G3| 吻合~10%。docs v2.11。+5 单元测试(test_transient_frequency.py)。安全点 commit 336f6cb。src/ 未变(R1)。 |
| 2026-06-07 | P12 | Zip (主 session) | (pending) | ✅ DONE | ~250K | 核函数 σ_t 旋钮 + Richardson 外推 + 非对角(sim)提取。诊断 exp 高阶对角偏差(k₃ 偏低~15%):根因 (a) VZ 探针 σ_t=2·dt 太宽涂抹非对角,(b) FD mesolve 默认容差 noise/h³ 主导。修复:`_extract_kn_omega` FD 容差→atol=1e-12/rtol=1e-10;新增 `probe_sigma_t`(None→2·dt 零回归)+ `richardson`/`richardson_sigmas`(σ_t→0 外推,G₃/G₃_sim 0.84→0.96);`extract_off_diagonal=True`(仅 sim)经 `_heisenberg_kernels_offdiag` 产出 n 维核(order≤3);`KernelResult.off_diagonal` + n 维 save/load;exp+offdiag 抛 NotImplementedError;`estimate_full` order≥2 补 `_validate_inputs`;transient.py Wiener/Hammerstein 对 ndim>1 抛 ValueError。docs v2.10。+8 单元测试。安全点 commit 58fb2d9。src/ 未变(R1)。G_α 直接拟合可行性:见 §4.6.2 注,P11 `_calibrate_g3_taylor` 已实现,本次未改。 |
