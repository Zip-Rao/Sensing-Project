...
$$
由于一般情况下，$H_0(t)$和$H_{\text{ctrl}}(t)$不对易，无法直接分解为两部分的演化算符乘积  $$
U(t, 0) \neq U_0(t, 0) U_{\text{ctrl}}(t, 0)$$

下面值考虑控制脉冲演化
$$
U_{ctrl}(t, 0) = \mathcal{T}\exp\left(-i\int_0^t H_{\text{ctrl}}(t') dt'\right)
$$
则总演化可以写为
$$
U(t, 0) = U_{ctrl}(t, 0)U_I(t, 0)
$$

代入Shroedinger方程，得到交互系的演化算符满足
$$  
i\frac{d}{dt}U_I(t, 0) = \frac{1}{2}U_{ctrl}^\dagger(t, 0)\,\sigma_z\,U_{ctrl}(t, 0)\,\delta\omega(t)\,U_I(t, 0) = V_I(t) U_I(t, 0) 
$$
定义有效$\sigma_z$为
$$
Z_c(t) = U_{ctrl}^\dagger(t, 0)\,\sigma_z\,U_{ctrl}(t, 0)
$$
则总演化为
$$
U(t, 0) = U_{ctrl}(t, 0) \mathcal{T}\exp\left(-\frac{i}{2}\int_0^t Z_c(t')\,\delta\omega(t') dt'\right)
$$


#### 2. 小失谐微扰展开

当 $|\delta\omega| \ll \Omega$ 时，失谐项可视为对理想脉冲动力学的微扰。对$U_I$作Dyson展开：

$$
U_I(T, 0) = I - i\int_0^T V_I(t')dt' - \int_0^T dt' \int_0^{t'} dt'' V_I(t')V_I(t'') + \cdots
$$
保留至一阶项，代入$V_I(t)$，并左乘$U_{ctrl}(T, 0)$，得
$$
\begin{aligned}
U(T, 0) &\approx U_{ctrl}(T, 0) \left[I - \frac{i}{2}\int_0^T U_{ctrl}^\dagger(t', 0)\,\sigma_z\,U_{ctrl}(t', 0)\,\delta\omega(t') dt'\right] \\
&= U_{ctrl}(T, 0) - \frac{i}{2}\int_0^T U_{ctrl}(T, t')\,\sigma_z\,U_{ctrl}(t', 0)\,\delta\omega(t') dt'
\end{aligned}
$$
初始态 $|0\rangle$ 经过脉冲序列后的末态为

$$
|\psi\rangle = U(T, 0)|0\rangle \approx U_{ctrl}(T, 0)|0\rangle - \frac{i}{2}\int_0^T U_{ctrl}(T, t')\,\sigma_z\,U_{ctrl}(t', 0)|0\rangle\,\delta\omega(t') dt'
$$

设测量算符为$M$，测量期望为
$$
\braket{M} = \langle\psi|M|\psi\rangle = \braket{M}_0 + \Delta\braket{M}
$$

其中
$$
\begin{aligned}
\braket{M}_0 &= \langle 0|U_{ctrl}^\dagger(T, 0) M U_{ctrl}(T, 0)|0\rangle \\
&= \frac{1}{2} 
\end{aligned}
$$

$$
\Delta \braket{M} = \braket{0|\delta U^\dagger M U_{ctrl}|0} + \braket{0|U_{ctrl}^\dagger M \delta U|0}
$$

对于第二项，有
$$
\braket{0|U_{ctrl}^\dagger M \delta U|0} = -\frac{i}{2  }\int_0^T \braket{0|U_{ctrl}^\dagger M U_{ctrl}(T, t')\,\sigma_z\,U_{ctrl}(t', 0)|0}\,\delta\omega(t') dt' = -\frac{i}{2}\int_0^T A_M \delta \omega(t') dt'
$$
$$
A_M = \braket{0|U_{ctrl}^\dagger M U_{ctrl}(T, t')\,\sigma_z\,U_{ctrl}(t', 0)|0}
$$
第一项为其复共轭，则
$$
\Delta \braket{M} = \int_0^T k_M(t')\,\delta\omega(t') dt'
$$


其中 $k_M(t)$ 即为**核函数（kernel / filter function）**：

$$k_M(t) =  \Im A_M(t) = \Im\Bigl[\langle 0|U_{ctrl}^\dagger(T,0)\,\sigma_z\,U_{ctrl}(T,t)\,\sigma_z\,U_{ctrl}(t,0)|0\rangle\Bigr]
$$
...
对于 $R_y(\pi/2)$—$R_x(\pi/2)$ 序列（两个 $\pi/2$ 首尾相连，中间无间隙），哈密顿量为
$$
H_{ctrl}(t) = \begin{cases} \Omega(t) \sigma_y/2, & 0 < t < T_{\pi/2} \\ \Omega(t) \sigma_x/2, & T_{\pi/2} < t < 2T_{\pi/2} \\ 0, & \text{else} \end{cases}
$$
包络满足
$$
\int_0^{T_{\pi/2}} \Omega(t) dt = \int_{T_{\pi/2}}^{2T_{\pi/2}} \Omega(t) dt = \pi/2
$$
设
$$
...
则在第一个脉冲内，有
$$
U_{ctrl}(t, 0) = R_y(\theta_y(t)), \quad t < T_{\pi/2}
$$
$$
U_{ctrl}(T, t) = R_x(\pi/2)R_y(\pi/2-\theta_y(t))
$$
代入核函数公式，有
$$
K_e(t) = -\frac{1}{2}\sin\theta_y(t) \quad t < T_{\pi/2}
$$

在第二个脉冲内，有
$$
U_{ctrl}(t, 0) = R_x(\theta_x(t))R_y(\frac{\pi}{2}), \quad T_{\pi/2} < t < 2T_{\pi/2}
$$
$$
U_{ctrl}(T, t) = R_x(\pi/2-\theta_x(t))
$$
代入核函数公式，有
$$
K_e(t) = -\frac{1}{2}\cos\theta_x(t) \quad T_{\pi/2} < t < 2T_{\pi/2}
$$
- 方波包络
有
$$
\theta_y(t) = \frac{\pi t}{2T_{\pi/2}}, \quad \theta_x(t) = \frac{\pi (t - T_{\pi/2})}{2T_{\pi/2}}
$$
$$
k(t+T_{\pi/2}) = \begin{cases}
...
由核函数满足的方程
$$
\delta p_e  =\int k(t') \delta\omega(t') dt'
$$
可以得到核函数的变分定义：
$$
k(t) = \frac{\delta p_e}{\delta \delta \omega(t)}\bigg|_{\delta\omega \to 0}
$$
...
则有
$$
\delta p_e = \int k(t')\delta \omega(t') dt' \approx k(t_j) \cdot \varepsilon \cdot \sqrt{2\pi}\sigma
$$
...
#### 5b. flux 核函数 vs omega 核函数

前面推导用的扰动量是**频率失谐** $\delta\omega(t)$，得到的核函数 $k^{(\omega)}(t)$ 满足

$$
\Delta p_e(t_d) = \int k^{(\omega)}(t-t_d)\,\delta\omega(t)\,dt,\qquad [k^{(\omega)}]=\text{rad}^{-1}\cdot\text{ns}^{-1}
$$

但实验/工程中真正可控的物理量往往是**磁通** $\delta\Phi(t)$（DC bias、Z 线脉冲），而频率失谐通过色散关系产生：

$$
\delta\omega(t)=\kappa\,\delta\Phi(t)+\tfrac12\kappa'\,\delta\Phi(t)^2+\cdots,\qquad \kappa=\left.\tfrac{d\omega}{d\Phi}\right|_{\Phi_w}
$$

把它代入 Volterra 展开，等价地可以**直接定义 flux 核函数**：

$$
\Delta p_e(t_d)=\int k^{(\Phi)}(t-t_d)\,\delta\Phi(t)\,dt+\frac12\!\iint k_2^{(\Phi)}(t_1,t_2)\delta\Phi(t_1)\delta\Phi(t_2)\,dt_1dt_2+\cdots
$$

$k^{(\Phi)}$ 的单位是 $\Phi_0^{-1}\cdot\text{ns}^{-1}$。两套核函数通过 $\kappa$ 联系。

**线性色散**（$\kappa'=0$）：

$$
\boxed{\,k_n^{(\Phi)}(t)=\kappa^n\,k_n^{(\omega)}(t)\,}
$$

**非线性色散**（$\kappa'\neq 0$）需要重新收集 $\delta\Phi$ 的幂次：

$$
k_1^{(\Phi)}=\kappa\,k_1^{(\omega)}
$$

$$
k_2^{(\Phi)}(t_1,t_2)=\kappa^2\,k_2^{(\omega)}(t_1,t_2)+\tfrac12\kappa'\,k_1^{(\omega)}(t_1)\,\delta(t_1-t_2)
$$

$$
k_3^{(\Phi)}=\kappa^3\,k_3^{(\omega)}+\tfrac32\kappa\kappa'\,k_2^{(\omega)}\!\circ\!\delta+\tfrac16\kappa''\,k_1^{(\omega)}\!\circ\!\delta\!\circ\!\delta
$$

（其中 $\circ\,\delta$ 表示对应时间变量收缩为同一点。）
...
| n=3 | 脉冲动力学的纯三阶非线性 | dispersion 三阶 + 多重交叉项 |

**怎么选**：

- $k^{(\omega)}$ **物理意义最干净**——只反映脉冲序列本身。验证理论公式（如 (3.12) 的方波 sin 形）、跨 qubit 比较脉冲质量、做频率标定（直接得 $\delta\omega$）应该用它。
- $k^{(\Phi)}$ 是**工程方便量**——把 dispersion 与 qubit 动力学的非线性都打包进同一个核，做磁通波形反演（cryoscope、阶跃响应、串扰矩阵）时一步到位。
- 当前 `sqc/reconstruction/kernel.py` 默认输出 $k^{(\Phi)}$（`mode='flux'`）；频率标定路径 [sqc/calibration/frequency.py:294-302](../sqc/calibration/frequency.py#L294) 通过乘 $1/\kappa$ 反推 $\delta\omega$，但这只在线性色散下严格成立。Phase 10 计划把 omega 模式作为一等公民，避免这一 workaround。
...
$$
H_{\rm stim}^{\rm sim}=\hbar\,\delta\omega(t-t_j)\,a^\dagger a
$$

$\delta\omega$ 取窄高斯。$a^\dagger a$ 在 2 能级时与 $-\sigma_z/2$ 差一个常数项（对差分无影响），但写成 $a^\dagger a$ 可以无缝推广到含 leakage 的多能级 transmon。

**exp 法**（"实验"路径）：等价于硬件可实现的两种刺激方式之一：

1. **Virtual Z**：在 $t_j$ 后所有子脉冲相位整体平移 $\pm\phi_z$。等价于在 $t_j$ 处插入瞬时 $\sigma_z\phi_z/2$ 旋转，即 $\delta\omega(t)=\phi_z\,\delta(t-t_j)$。代入 (3.5) 立得 $\Delta p_e(t_j)=\phi_z\,k_1^{(\omega)}(t_j)+O(\phi_z^2)$。
...
| exp Virtual Z | $k_1^{(\omega)}$ | 相同（含 leakage 自动） |
| exp 磁通刺激 | $k_1^{(\Phi)}$ | $k_1^{(\Phi)}=\kappa\,k_1^{(\omega)}$ |

非法组合 **(flux, sim)**：磁通刺激必须经过 $\omega(\Phi)$ 色散才能进入 qubit，没有 qubit 时 "flux 单位 + 不算 dispersion" 没有物理意义。如果想要纯理论 flux 核，等价于 omega+sim 的结果手乘 $\kappa$，没必要在 API 里再造一个伪法律组合。

**怎么选**：

- 验证脉冲序列的核函数公式是否正确 → **sim + omega**（与 (3.12) 这种闭式解直接对照）
- 频率标定（要的是 $\delta\omega$ 不是 $\delta\Phi$） → **exp + omega**（Virtual Z，含 qubit leakage 修正，最贴近实验）
- 磁通波形反演（cryoscope、阶跃响应、串扰矩阵） → **exp + flux**（一步到位，工程友好）
- 大信号、强非线性 → 任意路径 + 高阶 Volterra（参看 §8）

三个维度（mode = flux/omega、method = sim/exp、order = 1..N）正交，详细 API 设计见 [`idea/refactor/phase_10_kernel_extension_handbook.md`](refactor/phase_10_kernel_extension_handbook.md)。
...
前面的核函数 $k(t)$ 是测量结果对瞬时频率扰动 $\delta\omega(t)$ 的**一阶泛函导数**：

$$
k(t) = \left.\frac{\delta\langle M\rangle}{\delta\,\delta\omega(t)}\right|_{\delta\omega=0}
$$

只要积累相位

$$
\phi_{\rm acc} \;\sim\; \int |k(t)\,\delta\omega(t)|\,dt
\;\lesssim\; |\delta\omega|_{\max}\cdot T_{\rm eff},
\qquad T_{\rm eff}\equiv\int|k(t)|\,dt
$$

充分小（即 $\phi_{\rm acc}\ll 1$），则一阶近似 $\Delta\langle M\rangle\approx\int k(t)\delta\omega(t)dt$ 足以涵盖全部信号-探针响应。下列三种情形会破坏这一近似：

1. **大信号**：$|\delta\omega|_{\max}\cdot T_{\rm eff}\gtrsim 1$ 时，Bloch 矢量在 Bloch 球上转过的角度不再小，$\sin\phi$ 的非线性必须保留；
2. **频率-磁通映射的二阶以上项不可忽略**：$\delta\omega = \kappa\Phi+\tfrac12\kappa'\Phi^2+\cdots$，例如工作点偏离最优灵敏度点、或信号幅度较大；
3. **多频混频**：信号包含 $\omega_a,\omega_b$ 两个分量时，二阶响应会在 $\omega_a\pm\omega_b$ 处产生新的频率成分。

为了系统处理这些非线性，把 $\langle M\rangle$ 视为 $\delta\omega(\cdot)$ 的泛函，作 **Volterra 展开**：

$$
\Delta\langle M\rangle
= \sum_{n=1}^{\infty}\frac{1}{n!}
\int\!\cdots\!\int k_n(t_1,\ldots,t_n)
\prod_{j=1}^n\delta\omega(t_j)\,dt_j
$$

$$
k_n(t_1,\ldots,t_n)
= \left.\frac{\delta^n\langle M\rangle}{\delta\,\delta\omega(t_1)\cdots\delta\,\delta\omega(t_n)}\right|_{\delta\omega=0}
$$

按定义，$k_n$ 关于其参数**完全对称**：$k_n(t_1,\ldots,t_n)=k_n(t_{\sigma(1)},\ldots,t_{\sigma(n)})$ 对任意置换 $\sigma$ 成立——这是 Volterra 展开的对称化约定。$n=1$ 即上文的一阶核；$n=2$ 给出第一项非线性修正。

##### 8.2 交互系记号

沿用 §1 的交互系分解 $U(T,0)=U_{\rm ctrl}(T,0)U_I(T,0)$，并定义

$$
Z_c(t)=U_{\rm ctrl}^\dagger(t,0)\,\sigma_z\,U_{\rm ctrl}(t,0),\qquad
W(t)=\tfrac{1}{2}Z_c(t)
$$

则交互系扰动哈密顿量 $V_I(t)=\delta\omega(t)\,W(t)$。把测量算符也搬到理想控制框架：

$$
Q = U_{\rm ctrl}^\dagger(T,0)\,M\,U_{\rm ctrl}(T,0)
$$

期望值改写为

$$
\langle M\rangle=\langle 0|U_I^\dagger(T,0)\,Q\,U_I(T,0)|0\rangle
$$

Dyson 级数（标准时序约定 $t_1>t_2>\cdots$，左侧算符对应较晚时间）：

$$
U_I(T,0)
= I
- i\int_0^T\!\!dt_1\,\delta\omega(t_1)W(t_1)
- \int_0^T\!\!dt_1\!\int_0^{t_1}\!\!dt_2\,
\delta\omega(t_1)\delta\omega(t_2)W(t_1)W(t_2)
+\cdots
$$

##### 8.3 一阶核：算符表达与等价性

代入 $\langle M\rangle$，保留 $\delta\omega$ 的一阶项：
...
**实数性**：$W,Q$ 厄米 ⇒ $[W,Q]$ 反厄米 ⇒ 其期望为纯虚 ⇒ $i\times$（纯虚）= 实数。$k_1\in\mathbb{R}$。

**与前文 $k_M=\Im[\cdots]$ 写法的等价性**：注意 $U_{\rm ctrl}(t,0)U_{\rm ctrl}^\dagger(T,0)=U_{\rm ctrl}^\dagger(T,t)$，故

$$
\langle 0|Q\,Z_c(t)|0\rangle
= \langle 0|U_{\rm ctrl}^\dagger(T,0)\,M\,U_{\rm ctrl}(T,t)\,\sigma_z\,U_{\rm ctrl}(t,0)|0\rangle =: G(t)
$$

并且 $\langle 0|Z_c(t)\,Q|0\rangle=G^*(t)$。代入：

$$
k_1(t)=\tfrac{i}{2}\langle 0|[Z_c,Q]|0\rangle
=\tfrac{i}{2}\bigl(G^*-G\bigr)
=\Im\,G(t)
$$

恰为前文表达式 $k_M(t)=\Im[\langle 0|U_{\rm ctrl}^\dagger(T,0)M\,U_{\rm ctrl}(T,t)\sigma_z U_{\rm ctrl}(t,0)|0\rangle]$。两种写法等价。
...
$$
\Delta^{(2)}\langle M\rangle
= \langle 0|\underbrace{U_I^{(2)\dagger}Q}_{\text{(a)}}+\underbrace{Q\,U_I^{(2)}}_{\text{(b)}}+\underbrace{U_I^{(1)\dagger}Q\,U_I^{(1)}}_{\text{(c)}}|0\rangle
$$
...
$$
-W(t_2)W(t_1)Q-Q\,W(t_1)W(t_2)+W(t_1)Q\,W(t_2)+W(t_2)Q\,W(t_1)
= -[W(t_2),[W(t_1),Q]]
$$
...
$$
\Delta^{(2)}\langle M\rangle
= -\int_{t_1>t_2}\!\!dt_1\,dt_2\,
\delta\omega(t_1)\delta\omega(t_2)\,
\langle 0|[W(t_2),[W(t_1),Q]]|0\rangle
$$
...
k_2(t_1,t_2)\delta\omega(t_1)\delta\omega(t_2)\,dt_1\,dt_2
$$

由对称性 $k_2(t_1,t_2)=k_2(t_2,t_1)$，对称化积分恰好两倍于时序楔形区域，因此
...
$$
k_n^{\rm ord}(t_{(1)},\ldots,t_{(n)})
= i^n\,\bigl\langle 0\bigl|\bigl[W(t_{(n)}),\bigl[\cdots,[W(t_{(1)}),Q]\cdots\bigr]\bigr]\bigr|0\bigr\rangle
$$

（n=1: $k_1=i\langle 0|[W,Q]|0\rangle$；n=2: $k_2^{\rm ord}=i^2\langle 0|[W(t_{(2)}),[W(t_{(1)}),Q]]|0\rangle=-\langle 0|[W(t_<),[W(t_>),Q]]|0\rangle$，与上文一致。）
...
$$
\Delta p_e
= \int k_1(t)\delta\omega(t)\,dt
+ \frac{1}{2}\!\int\!\!\int\! k_2(t_1,t_2)\delta\omega(t_1)\delta\omega(t_2)\,dt_1dt_2
+ \cdots
$$

##### 8.6 数值 $\delta$ 刺激法的二阶推广

对两个采样点 $t_i,t_j$ 同时施加窄高斯频率刺激：

$$
\delta\omega(t)=\varepsilon_i\,g_i(t-t_i)+\varepsilon_j\,g_j(t-t_j),\qquad
A_i=\!\int\!g_i(t)\,dt
$$
...
$$
\Delta\langle M\rangle
\approx \sum_{\alpha\in\{i,j\}}\!\varepsilon_\alpha A_\alpha\,k_1(t_\alpha)
+\!\!\sum_{\alpha,\beta\in\{i,j\}}\!\!\tfrac{1}{2}\varepsilon_\alpha\varepsilon_\beta A_\alpha A_\beta\,k_2(t_\alpha,t_\beta)
+\cdots
$$
...
$$
\boxed{\,k_2(t_i,t_j)\approx
\frac{P_{++}-P_{+-}-P_{-+}+P_{--}}{4\,\varepsilon_i\varepsilon_j A_iA_j}\,}
$$

**对角点 $i=j$**——单点二阶中心差分：

$$
\boxed{\,k_2(t_i,t_i)\approx
\frac{P_{+\varepsilon}-2P_0+P_{-\varepsilon}}{\varepsilon^2 A_i^2}\,}
$$

**误差控制**：$\varepsilon$ 的选取要同时压低 $O(\varepsilon^4)$ 截断差分误差与放大读出噪声方差。可扫描多个 $\varepsilon$ 做 Richardson 外推或线性回归提高鲁棒性。

##### 8.7 适用边界与实用建议

- **维度灾难**：一阶核是一维函数（$N$ 个采样点），二阶核是二维矩阵（$N^2$）；$N\sim 10^2$ 时 $k_2$ 已到 $10^4$ 量级，三阶则到 $10^6$。**实际中很少把 $k_2$ 直接用作反卷积矩阵**——更多用于估计与校核。
- **与 Hammerstein-Wiener 模型的对应**：当非线性主要来自频率-磁通映射的静态非线性 $g(\Phi)=\kappa\Phi+\tfrac12\kappa'\Phi^2+\cdots$、且脉冲序列保持线性的 $k_1$ 响应时，二阶 Volterra 核退化为对角形式
  $$
  k_2^{\rm HW}(t_1,t_2)=\kappa'\,k_1(t_1)\,\delta(t_1-t_2)
  $$
  即 "$\Phi^2$ 项 $\to$ 一阶核做积分"。检查测得的 $k_2$ 是否近似对角，是判断 Hammerstein-Wiener 假设是否成立的有效手段。
...
- **典型用法**：
  ...
  - 验证**Hammerstein-Wiener 模型**有效性：测得 $k_2$ 是否近似对角；
