...
为什么需要第二种更新:每轮测频是**局部鉴频器**,只有
$|\delta|=|\hat f_q-f_d|<\Delta_\mathrm{lin}$ 时测得的失谐 $\hat\delta$ 才可信
(尤其省成本的瞬态法)。若 $f_d$ 钉在甜点而搜索探到很远的磁通点,测得误差就有偏,
闭环可能发散;驱动频率更新专门消除这一偏差。
...
配套字段:`sensitivity_source`(`"secant"`——相邻两点无模型斜率,默认;或 `"model"`
——解析 `qubit.sensitivity(V)`)供 `"track"` 预测;`linear_range`($\Delta_\mathrm{lin}$)
配 `rho`(默认 0.6),当 $|\delta|>\rho\,\Delta_\mathrm{lin}$ 时在每条 history 行与
`stop_predicate` 的 state 里置 `out_of_range`——调用方可监听该信号触发更宽的**重捕获**
阶段。每条 `history` 行现在也带 `omega_d` 与 `delta` 供诊断。

`converge_streak`(默认 1)要求**连续**这么多轮 $|e|\le\epsilon$ 才判收敛——原地
重测去抖,LOCKED 阶段抗噪用。`omega_d_seed` 提供首轮 `"track"` 驱动
$f_{d,0}=\hat f_{q,0}$;缺了它,新开的 track 段在第 0 轮回落甜点,目标远时致命
(瞬态直接看到整段偏移)。

### 四阶段状态机

把上面两种更新与不同的测频协议、`drive_policy` 组合起来,整个闭环标定对应一个四阶段
状态机。每个阶段是一次 `SinglePointFrequencyCalibration`,由下方
`FrequencyCalibrationWorkflow` 按序串联(每段以前一段的最优磁通和频率估计作初值):
...
- **REACQUIRE** 不是自动跳转,而是**编排层的响应**:当 `linear_range`
  ...
  `stop_predicate`)插入一个新的宽范围阶段重估频率。每条 `history` 行带 `omega_d`
  与 `delta` 供诊断。

支撑上述状态机的字段:`sensitivity_source`(`"secant"` 相邻两点无模型斜率,默认;或
`"model"` 解析 `qubit.sensitivity(V)`)供 TRACKING 的预测;`converge_streak`(默认 1)
要求**连续**这么多轮 $|e|\le\epsilon$ 才判收敛,LOCKED 阶段抗噪用;`omega_d_seed` 给
首轮 `"track"` 驱动 $f_{d,0}=\hat f_{q,0}$,缺了它新开的 TRACKING 段在第 0 轮回落甜点,
目标远时致命(瞬态直接看到整段偏移)。

`FrequencyCalibrationWorkflow`(下方)就是这套状态机的编排实现:`CalibrationStage`
暴露 `drive_policy`、`sensitivity_source`、`linear_range`/`rho`、`converge_streak`
与 `seed_drive_from_prev`(默认开:把上一段的频率估计作为 `omega_d_seed` 交给 TRACKING
段)。见 notebook §B2.2c:−20 MHz 远目标下,纯单段瞬态停在错误磁通,而状态机收到 kHz 级。
...
标定任务的调度器(`@dataclass`):维护带依赖的命名标定任务注册表,按序执行。
