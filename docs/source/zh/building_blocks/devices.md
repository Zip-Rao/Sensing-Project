...
`devices` 层是全栈 cQED 架构的最底层,只建模物理量子器件本身:超导 Transmon
比特、读出/耦合谐振腔、以及把它们组装起来的芯片拓扑与耦合系统。这一层只描述
物理(能量、频率、Hilbert 空间、Hamiltonian、耗散算符),不涉及控制脉冲、时间
演化或重建逻辑;那些职责在上面的 `control`、`simulation` 等层。

一条贯穿本层的设计约束:器件描述物理,但不存储实验状态。要表示受扰动的器件
(如处于非零磁通的比特),用返回新对象的方法(`QubitSpec.with_flux`),而不是
就地修改。`QubitSpec` 因此是 frozen dataclass。
...
**单位约定**:`EC`、`EJ` 以 rad·GHz 传入(已含 $2\pi$ 因子),磁通以 $\Phi_0$
为单位,时间以 ns 计,$\hbar = 1$。所以 $E_C/h = 200\,\mathrm{MHz}$ 对应传参
`EC = 2*np.pi*0.2`;`frequency()` 返回的也是 rad·GHz(即角频率 $\omega_{01}$),
除以 $2\pi$ 得到以 GHz 计的物理频率 $f_{01}$。全站默认器件参数
...
所有物理器件的公共契约,是本层的扩展点。子类只描述物理、不存实验状态,并实现
以下抽象成员:
...
一个可调频 Transmon 的参数描述,也是本层的参数内核:`@dataclass(frozen=True)`,
...
进入实验流程的运行时器件对象。它内部持有一个 `QubitSpec`,并把 QuTiP 算符、
...
`simulation` 各层默认接收 `TransmonQubit`,`sqc.config.CONFIG` 产出的默认比特也是
此类型。`QubitSpec` 只描述物理参数;要进入实验流程,用 `TransmonQubit`。
...
`@dataclass`,充当整芯片所有器件的容器,并负责把单器件算符嵌入(lift)完整
Hilbert 空间。继承 `Device`。
...
- `hamiltonian_static() -> Qobj`：各器件静态 Hamiltonian 之和(注:基础实现不含耦合项,耦合系统请用 `CoupledSystem`)。
...
- 要加自定义器件类型(如 fluxonium、多结比特),继承 `Device` 抽象基类,实现
  ...
  并且不存实验状态。完整扩展指南见 {doc}`../extending`。
