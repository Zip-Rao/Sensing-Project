#!/usr/bin/env python3
"""Assemble the three editable Fig. 1 SVG panels into one publication SVG."""
...
PANELS = [
    ...
    ("fig1b-v6-editable.svg", "Panel b - probe timing", 98.0, 7.0, 80.0),
    ("fig1d-v8-editable.svg", "Panel c - calibration workflow", 24.0, 61.0, 132.0),
]
...
def main() -> None:
    ...
    ET.SubElement(root, f"{{{SVG}}}title").text = "Fig. 1: system, probes, and calibration workflow"
    ET.SubElement(root, f"{{{SVG}}}desc").text = (
        "Editable two-column assembly of panels (a)-(c). Panel groups are separate Inkscape layers."
    )
    ...
    for text, x, y in (("(a)", 0.8, 3.4), ("(b)", 98.8, 9.8), ("(c)", 24.8, 64.4)):
        ET.SubElement(labels, f"{{{SVG}}}text", {"x": str(x), "y": str(y)}).text = text

    out = OUT / "fig1-system-protocol-editable.svg"
