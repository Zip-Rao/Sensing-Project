...
% are recomputed from the analytic transmon dispersion, rather than taken from
% the estimate used by the feedback controller. This provides deterministic
% simulation ground truth, not an independent experimental measurement.
...
%  baseline (red squares). Residuals are evaluated from the analytic
%  frequency--flux relation and therefore do not constitute \textsc{Verify}
%  measurements.}
...
% and reaches an analytically checked residual of 0.00519~MHz. Ramsey alone uses 4000 calls
...
%  \caption{Cost anatomy of closed-loop frequency calibration. (a) Analytic
...
%  calls and analytically checked final residuals. The full-kernel transient
...
% These closed-loop data isolate the seeded \textsc{Track} mechanism. They do
% not execute the complete \textsc{Acquire--Track--Verify--Lock} protocol, and
% they contain neither finite-shot verification statistics nor long-term
% monitoring records. In particular, the controller's consecutive local hits
% are not reinterpreted as independent \textsc{Verify} outcomes. The full state
% machine specifies how those measurements are to be acquired and separated,
% but its hardware-level performance is outside the evidence reported here.
